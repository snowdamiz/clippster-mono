...
| organization_id | bigint FK | Reference to organization |
| uploaded_by_user_id | bigint FK | Admin who uploaded |
| name | string | Display name |
| description | text | Optional context/caption |
| url | text | R2 storage URL |
| thumbnail_url | text | Video thumbnail |
| duration | decimal | Duration in seconds (max 180) |
| file_size | bigint | Size in bytes |
| share_with_all | boolean | True = all members, False = specific |
| branding_config | map/jsonb | Per-aspect-ratio branding settings (see below) |
| branding_required | boolean | If true, branding MUST be applied on export |
| expires_at | utc_datetime | Auto-set to inserted_at + 7 days |
| timestamps | | inserted_at, updated_at |

### Branding Config Structure

The `branding_config` field is a JSON object with per-aspect-ratio settings:

```json
{
  "16:9": {
    "watermark_id": "uuid-of-org-asset",
    "watermark_settings": {
    ...
    "intro_id": "uuid-of-intro-asset",
    "outro_id": "uuid-of-outro-asset"
  },
  "9:16": {
    "watermark_id": "uuid",
    "watermark_settings": { ... },
...
When `branding_required` is `true`, the clipper cannot disable the org branding when exporting - watermarks, intros, and outros are automatically applied based on the selected aspect ratio.
...
|--------|------|-------------|
| id | bigint | Primary key |
| shared_clip_id | bigint FK | Reference to shared clip |
| user_id | bigint FK | Recipient member |
| viewed_at | utc_datetime | When member first viewed |
| downloaded_at | utc_datetime | When member downloaded |
| posted_at | utc_datetime | When member posted |
| timestamps | | inserted_at, updated_at |
...
- Includes `branding_config` map field and `branding_required` boolean
...
|--------|----------|-------------|--------|
| POST | `/organizations/:id/shared-clips` | Upload and share clip | Admin |
| GET | `/organizations/:id/shared-clips` | List all shared clips | Admin |
| GET | `/user/shared-clips` | Get clips shared with current user | Member |
| GET | `/organizations/:id/shared-clips/:clip_id` | Get single clip | Member |
| PUT | `/organizations/:id/shared-clips/:clip_id/branding` | Update branding config | Admin |
| DELETE | `/organizations/:id/shared-clips/:clip_id` | Delete shared clip | Admin |
| POST | `/shared-clips/:clip_id/mark-viewed` | Mark clip as viewed | Member |
| POST | `/shared-clips/:clip_id/mark-downloaded` | Record download action | Member |
| POST | `/shared-clips/:clip_id/post` | Post clip to social account | Member |
...
- **Mandatory branding**: If `branding_required` is true, clippers cannot disable org branding on export
...
1. **Admin uploads shared clip** and optionally configures branding per aspect ratio
2. **Branding config** specifies watermark, intro, and outro assets for each aspect ratio (16:9, 9:16, 1:1)
3. **Assets reference** existing `organization_assets` (watermarks, intros, outros already uploaded to org)
4. **On export**, if `branding_required` is true:
   - Clipper selects aspect ratio for export
   - System automatically applies the corresponding watermark/intro/outro from branding config
   - Clipper cannot disable or modify org branding
5. **If branding not required**, clipper can choose to apply or skip branding

### Branding UI (Admin)

When sharing a clip, admin sees:
- Toggle: "Require branding on export" (default: true)
- Per-aspect-ratio configuration:
  - Select watermark from org assets
  - Configure watermark position, opacity, scale
  - Select intro video from org assets
  - Select outro video from org assets
- Preview of how branding will look

### Branding Integration (Client)

When clipper exports a shared clip:
1. Check if `branding_required` is true
2. Get `branding_config` for selected aspect ratio
3. Fetch watermark/intro/outro assets (download if not cached locally)
4. Pass branding settings to clip builder (same pipeline as creator profile branding)
5. Lock branding controls in UI if required