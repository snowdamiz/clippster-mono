...
pub async fn detect_focal_points(
    ...
    println!(
        "[FocalDetection] Starting focal point detection for: {}",
        video_path
    );
    ...
    println!(
        "[FocalDetection] Video info: {}x{}, duration: {:.2}s, fps: {:.2}",
    ...
    println!(
        "[FocalDetection] Detected {} focal points",
        focal_points.len()
    );
    Ok(focal_points)
}
...
async fn get_video_info_for_focal_detection(
    ...
    let (width, height) =
        parse_video_dimensions(&stderr).ok_or("Failed to parse video dimensions")?;

    // Parse duration
    let duration = parse_duration(&stderr).ok_or("Failed to parse video duration")?;

    // Parse FPS
    let fps = parse_fps(&stderr).unwrap_or(30.0); // Default to 30fps if can't parse
...
async fn run_cropdetect_analysis(
    ...
    let filter_string = format!(
        "select='not(mod(n\\,{}))',cropdetect=24:16:0",
        frame_interval
    );

    println!(
        "[FocalDetection] Running FFmpeg cropdetect with filter: {}",
        filter_string
    );
    ...
    let focal_points = parse_cropdetect_output(&stderr, video_info, interval_seconds as f64);
    ...
    for line in output.lines() {
        if let Some(captures) = re.captures(line) {
            ...
            let x2: f64 = captures
                .get(2)
                .and_then(|m| m.as_str().parse().ok())
                .unwrap_or(video_info.width as f64);
            let y1: f64 = captures
                .get(3)
                .and_then(|m| m.as_str().parse().ok())
                .unwrap_or(0.0);
            let y2: f64 = captures
                .get(4)
                .and_then(|m| m.as_str().parse().ok())
                .unwrap_or(video_info.height as f64);
