...
pub fn get_video_file_metadata(video_path: &str) -> Result<(u64, u64), String> {
    ...
    let metadata = std::fs::metadata(&local_path).map_err(|e| {
        format!(
            "Failed to get video file metadata for {}: {}",
            local_path, e
        )
    })?;
...
pub fn get_waveform_cache_file_path(video_path_hash: &str) -> Result<std::path::PathBuf, String> {
    ...
    Ok(paths
        .temp
        .join(format!("waveform_cache_{}.json", video_path_hash)))
}
...
pub fn get_audio_cache_file_path(video_path_hash: &str) -> Result<std::path::PathBuf, String> {
    ...
    Ok(paths
        .temp
        .join(format!("audio_cache_{}.mp3", video_path_hash)))
}

// Save waveform data to file cache
pub async fn save_waveform_to_file_cache(
    video_path_hash: &str,
...
pub async fn load_waveform_from_file_cache(
    video_path_hash: &str,
    ...
    let mut file =
        File::open(&cache_file_path).map_err(|e| format!("Failed to open cache file: {}", e))?;
...
#[tauri::command]
pub async fn get_cached_waveform(video_path: String) -> Result<Option<WaveformData>, String> {
    ...
    match load_waveform_from_file_cache(&video_path_hash).await {
        ...
        Err(e) => {
            println!(
                "[Rust] Error checking file cache: {}, proceeding with generation",
                e
            );
        }
    }
...
#[tauri::command]
pub async fn clear_waveform_cache(video_path: String) -> Result<(), String> {
...
pub async fn extract_audio_waveform(
    app: tauri::AppHandle,
    #[allow(non_snake_case)] videoPath: String,
    #[allow(non_snake_case)] _targetSamples: Option<u32>,
    #[allow(non_snake_case)] forceRegenerate: Option<bool>,
    ...
    if !force {
        match get_cached_waveform(video_path.clone()).await {
            ...
            Err(e) => {
                println!(
                    "[Rust] Error checking cache: {}, proceeding with generation",
                    e
                );
            }
        }
    ...
    let temp_wav_path = paths.videos.join(format!(
        "temp_waveform_{}.wav",
    ...
    println!(
        "[Rust] Extracting audio directly to {}Hz WAV: {}",
    ...
    let extract_output = shell
        ...
        .args([
            ...
            "-ar",
            &WAVEFORM_SAMPLE_RATE.to_string(), // 16kHz sample rate (sufficient for waveform)
            "-ac",
            "1",  // Mono
            "-y", // Overwrite output
    ...
    let duration_output = shell
        .sidecar("ffmpeg")
        .map_err(|e| format!("Failed to get ffmpeg sidecar: {}", e))?
        .args(["-nostdin", "-i", &local_video_path, "-f", "null", "-"])
        .output()
        .await
    ...
    if let Err(e) = std::fs::remove_file(&temp_wav_path) {
        eprintln!(
            "[Rust] Warning: Failed to remove temporary WAV file {}: {}",
            temp_wav_path.display(),
            e
        );
    } else {
    ...
    println!(
        "[Rust] Waveform extraction completed. Generated {} peaks.",
        waveform_data.peak_count
    );

    // Save to cache for future use
    let raw_video_id = format!("waveform_{}", video_path_hash);
    if let Err(e) =
        save_waveform_to_cache(video_path.clone(), raw_video_id, waveform_data.clone()).await
    {
...
fn process_wav_file(
    ...
    println!(
        "[Rust] First 20 bytes of audio data: {:?}",
        &audio_data[..20.min(audio_data.len())]
    );
    ...
    for i in 0..10.min(audio_data.len() / 2) {
        ...
        println!(
            "[Rust]   Sample {}: raw={}, normalized={:.6}",
            i, raw_i16, normalized
        );
    }
    ...
    println!(
        "[Rust] Raw sample range: min={}, max={} (i16 range is -32768 to 32767)",
        min_raw, max_raw
    );
    ...
    println!(
        "[Rust] Audio duration from samples: {:.2}s, expected duration: {:.2}s",
    ...
    if (audio_duration_from_samples - duration).abs() > 1.0 {
        println!(
            "[Rust] WARNING: Audio duration mismatch! Samples suggest {:.2}s but video is {:.2}s",
            audio_duration_from_samples, duration
        );
    }
    ...
    println!(
        "[Rust] Raw audio stats: min={:.4}, max={:.4}, non-zero samples: {} ({:.1}%)",
        min_sample,
        max_sample,
        non_zero_count,
    ...
    println!(
        "[Rust] Checking audio at specific time points (sample_rate={}):",
        sample_rate
    );
    for time_sec in [0, 2, 3, 4, 5, 10, 30, 60, 120, 180, 240] {
        if time_sec < audio_duration_from_samples as usize {
            ...
            if sample_idx < samples.len() {
                ...
                println!(
                    "[Rust]   At {}s (sample_idx={}): min={:.4}, max={:.4}",
                    time_sec, sample_idx, min_val, max_val
                );
            }
        }
    }
    ...
    println!(
        "[Rust] Generating {} absolute peaks from {} samples",
        actual_peaks, num_samples
    );
    ...
    println!(
        "[Rust] Generated {} peaks covering {} samples",
        peak_count, num_samples
    );
    ...
    println!(
        "[Rust] Global max peak: {:.4} at index {} (time: {:.1}s)",
        global_max, max_peak_idx, max_peak_time
    );

    // Debug: Print peaks at specific time points where we know there's audio
    let peaks_per_second = actual_peaks as f64 / duration;
    println!(
        "[Rust] Peaks at specific times (peaks_per_second={:.1}):",
        peaks_per_second
    );
    for time_sec in [2, 3, 4, 5, 60, 120, 240] {
        ...
        if peak_idx < peaks.len() {
            ...
            println!(
                "[Rust]   At {}s (peak {}): min={:.4}, max={:.4}",
                time_sec, peak_idx, p.min, p.max
            );
        }
    }
...
/// Extract audio peaks for a specific time range on-demand
/// This is used for long videos to avoid loading entire audio into memory
#[tauri::command]
pub async fn extract_audio_peaks_for_range(
    ...
    println!("[Rust] extract_audio_peaks_for_range called:");
    println!("[Rust]   video_path: {}", video_path);
    println!("[Rust]   start_time: {:.2}s", start_time);
    println!("[Rust]   duration: {:.2}s", duration);
    println!("[Rust]   num_peaks: {}", num_peaks);

    // Extract local file path from URL for FFmpeg
    let local_video_path = extract_local_path_from_url(&video_path)
        .map_err(|e| format!("Failed to extract local path from video URL: {}", e))?;
    ...
    println!(
        "[Rust] Extracting audio segment to: {}",
        temp_wav_path.display()
    );
    ...
    println!("[Rust] Audio segment extraction completed");

    // Process the WAV file to extract peaks
    let peaks = process_wav_file_to_peaks(&temp_wav_path, num_peaks).map_err(|e| {
        // Clean up temp file on error
        let _ = std::fs::remove_file(&temp_wav_path);
        format!("Failed to process WAV file: {}", e)
    })?;
    ...
    println!(
        "[Rust] Generated {} peaks for time range {:.2}s-{:.2}s",
        peaks.len(),
        start_time,
        start_time + duration
    );
...
/// Process WAV file to extract just the peaks (without full WaveformData structure)
fn process_wav_file_to_peaks(
    wav_path: &std::path::Path,
    target_peaks: u32,
) -> Result<Vec<WaveformPeak>, String> {
    use std::fs::File;
    ...
    loop {
        ...
        if chunk_id == b"fmt " {
            // Skip fmt chunk
            file.seek(SeekFrom::Current(chunk_size as i64))
                .map_err(|e| format!("Failed to skip fmt chunk: {}", e))?;
        } else if chunk_id == b"data" {
            data_size = chunk_size;
            break;
        } else {
