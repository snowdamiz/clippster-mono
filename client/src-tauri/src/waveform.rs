...
pub async fn extract_audio_peaks_for_range(
    ...
    let peaks = process_wav_file_to_peaks(&temp_wav_path, num_peaks)
        .map_err(|e| {
            // Clean up temp file on error
            let _ = std::fs::remove_file(&temp_wav_path);
    ...
    println!("[Rust] Generated {} peaks for time range {:.2}s-{:.2}s", peaks.len(), start_time, start_time + duration);
