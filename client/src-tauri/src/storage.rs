...
/// Tauri command to merge multiple video segments into a single file
#[tauri::command]
pub async fn merge_video_segments(
    app: tauri::AppHandle,
    ...
    println!("[Rust] Merging {} video segments into: {}", segment_paths.len(), output_path);

    if segment_paths.len() < 2 {
        return Err("Need at least 2 segments to merge".to_string());
    }
    ...
    let temp_dir = paths.temp.join(format!("merge_{}", uuid::Uuid::new_v4()));
    ...
    for segment_path in &segment_paths {
        // Escape single quotes and use proper format for FFmpeg concat
        let escaped_path = segment_path.replace("'", "'\\''");
        concat_content.push_str(&format!("file '{}'\n", escaped_path));
    }
    
    println!("[Rust] Concat file content:\n{}", concat_content);
    ...
    // Run FFmpeg concat with copy codec (no re-encoding, fast)
    let shell = app.shell();
    let output = shell
        .sidecar("ffmpeg")
        .map_err(|e| format!("Failed to get ffmpeg sidecar: {}", e))?
    ...
    let duration = crate::ffmpeg_utils::get_video_duration_sync(&app, &output_path)
        .await
        .unwrap_or(0.0);

    println!("[Rust] Merge complete: {} ({:.1}s, {} bytes)", output_path, duration, metadata.len());
