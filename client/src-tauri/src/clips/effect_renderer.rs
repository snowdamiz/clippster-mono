...
pub fn build_effect_filter(
    ...
    let filter = match effect.effect_type.as_str() {
        ...
        "blur" => {
            ...
            format!(
                "boxblur={}:{}:{}",
                radius as i32, radius as i32, enable_expr
            )
        }
        "motion-blur" => {
        ...
        "vhs" => {
            ...
            format!(
                "noise=alls={}:allf=t,rgbashift=rh=2:bh=-2:{}",
                noise_amount, enable_expr
            )
        }
        "crt" => {
        ...
        "pixelate" => {
            ...
            format!(
                "scale=iw/{}:ih/{}:flags=neighbor,scale=iw*{}:ih*{}:flags=neighbor:{}",
        ...
        "mirror-h" => {
            format!(
                "crop=iw/2:ih:0:0,split[left][right];[right]hflip[right];[left][right]hstack:{}",
                enable_expr
            )
        }
        "mirror-v" => {
            format!(
                "crop=iw:ih/2:0:0,split[top][bottom];[bottom]vflip[bottom];[top][bottom]vstack:{}",
                enable_expr
            )
        }
        ...
        "letterbox" => {
            ...
            format!(
                "drawbox=y=0:h={}:c=black:t=fill,drawbox=y=ih-{}:h={}:c=black:t=fill:{}",
        ...
        _ => {
            log::warn!(
                "Unknown effect type: {}, using null filter",
                effect.effect_type
            );
            "null".to_string()
        }
...
pub fn build_transition_filter(
    ...
    let xfade_type = match transition.transition_type.as_str() {
        ...
        _ => {
            log::warn!(
                "Unknown transition type: {}, using fade",
    ...
    let filter = format!(
        ...
        input1_label, input2_label, xfade_type, transition.duration, offset, output_label
    );
