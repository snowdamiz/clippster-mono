...
pub async fn get_video_info(
    app: &tauri::AppHandle,
    video_path: &str,
) -> Result<crate::ffmpeg_utils::VideoInfo, String> {
    // Check cache first
    {
    ...
    println!(
        "[Rust] Fetching video info from container header for: {}",
        video_path
    );
    let shell = app.shell();
    ...
    let output = shell
        ...
        .args(["-nostdin", "-hide_banner", "-i", video_path])
        .output()
        .await
...
fn parse_video_info_alternative(output: &str) -> Result<crate::ffmpeg_utils::VideoInfo, String> {
    ...
    let res_regex =
        Regex::new(r"(\d{3,5})x(\d{3,5})").map_err(|e| format!("Regex error: {}", e))?;
    ...
    match (width, height, &codec) {
        (Some(w), Some(h), Some(c)) => {
            ...
            Ok(crate::ffmpeg_utils::VideoInfo {
                width: w,
                height: h,
                codec: c.clone(),
                duration: None,
        ...
        _ => {
            println!(
                "[Rust] Failed to parse - width: {:?}, height: {:?}, codec: {:?}",
                width, height, codec
            );
