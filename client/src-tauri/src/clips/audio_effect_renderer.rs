...
pub fn build_audio_effect_filter(effect: &AudioEffectSettings) -> Result<String, String> {
    ...
    let filter = match effect.effect_type.as_str() {
        ...
        "limiter" => {
            ...
            let release = get_param_f64(&effect.parameters, "release", 50.0) / 1000.0;
            format!(
                "alimiter=limit={}dB:attack={}:release={}:{}",
        ...
        "bandpass" => {
            ...
            format!(
                "bandpass=f={}:width_type=q:w={}:{}",
                frequency, q, enable_expr
            )
        }
        "bass-boost" => {
        ...
        "distortion" => {
            ...
            format!(
                "acrusher=bits={}:mode=log:aa=1:samples={}:{}",
                bits, samples, enable_expr
            )
        }
        ...
        _ => {
            log::warn!(
                "Unknown audio effect type: {}, using anull filter",
                effect.effect_type
            );
            "anull".to_string()
        }
...
pub fn build_audio_effects_filter_chain(
    ...
    for effect in &sorted_effects {
        match build_audio_effect_filter(effect) {
            ...
            Err(e) => {
                log::warn!(
                    "Failed to build audio filter for effect {}: {}",
                    effect.id,
                    e
                );
            }
...
mod tests {
    ...
    fn test_compressor_effect() {
        let effect = AudioEffectSettings {
            ...
            intensity: 1.0,
            parameters: Some(
                serde_json::json!({"threshold": -20, "ratio": 4, "attack": 10, "release": 100}),
            ),
        };
