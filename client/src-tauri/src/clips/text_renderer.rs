...
fn get_style_for_ratio(overlay: &TextOverlaySettings, aspect_ratio: &str) -> Option<TextOverlayStyle> {
...
pub fn render_text_overlay_to_png(
    ...
    let style = get_style_for_ratio(overlay, aspect_ratio)
        .ok_or("No style found for overlay")?;
    ...
    let output_path = output_dir.join(format!("text_overlay_{}_{}.png", overlay.id, aspect_ratio.replace(":", "x")));
...
fn generate_text_svg(
    ...
    if let Some(ref chat_bubble) = style.chat_bubble {
        if chat_bubble.enabled {
            svg.push_str(&generate_chat_bubble_bg(
                center_x, center_y, &text, font_size as f64, style, chat_bubble
            )?);
        }
        ...
        let fill = if style.background_gradient.as_ref().map(|g| g.enabled).unwrap_or(false) {
    ...
    if style.border1_width > 0.0 {
        ...
        stroke_attr = format!(r#" stroke="{}" stroke-width="{}""#, border_color, style.border1_width);
    }
...
fn generate_gradient_def(id: &str, gradient: &super::types::GradientConfig) -> Result<String, String> {
...
fn generate_chat_bubble_bg(
    ...
    if tail_position != "none" {
        ...
        let tail_path = match tail_position {
            "left" => format!(
                "M {} {} L {} {} L {} {} Z",
                box_x, center_y - tail_size / 2.0,
                box_x - tail_size, center_y,
                box_x, center_y + tail_size / 2.0
            ),
            "right" => format!(
                "M {} {} L {} {} L {} {} Z",
                box_x + box_width, center_y - tail_size / 2.0,
                box_x + box_width + tail_size, center_y,
                box_x + box_width, center_y + tail_size / 2.0
            ),
            "bottom-left" => format!(
                "M {} {} L {} {} L {} {} Z",
                box_x + 20.0, box_y + box_height,
                box_x + 10.0, box_y + box_height + tail_size,
                box_x + 30.0, box_y + box_height
            ),
            "bottom-right" => format!(
                "M {} {} L {} {} L {} {} Z",
                box_x + box_width - 30.0, box_y + box_height,
                box_x + box_width - 10.0, box_y + box_height + tail_size,
                box_x + box_width - 20.0, box_y + box_height
            ),
            _ => String::new(),
        };
