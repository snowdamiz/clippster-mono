...
use super::types::{
    ClipWatermarkSettings, StickerSettings, SubtitleSettings, TextOverlaySettings, WhisperSegment,
};
...
pub fn build_sticker_overlay_filter(
    ...
    let overlay_filter = format!(
        ...
        sticker_label, x_pixels, y_pixels, sticker.start_time, sticker.end_time
    );
...
pub fn build_clip_watermark_overlay_filter(
    ...
    let (pos_x, pos_y, scale_pct, opacity_pct) =
        get_clip_watermark_config_for_ratio(watermark, aspect_ratio);
    ...
    let overlay_filter = format!(
        "[v][{}]overlay=x={}:y={}:enable='between(t,{},{})'",
        watermark_label, x_pixels, y_pixels, watermark.start_time, watermark.end_time
    );
...
pub fn build_complete_overlay_filter(
    ...
    for sticker in stickers {
        let filter = build_sticker_overlay_filter(
            sticker,
            video_width,
            video_height,
            aspect_ratio,
    ...
    for watermark in clip_watermarks {
        let filter = build_clip_watermark_overlay_filter(
            watermark,
            video_width,
            video_height,
            aspect_ratio,
