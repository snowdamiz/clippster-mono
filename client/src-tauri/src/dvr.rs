...
fn extract_init_segment(data: &[u8]) -> Option<Vec<u8>> {
    ...
    for i in 0..data.len().saturating_sub(3) {
        if data[i] == 0x1F && data[i + 1] == 0x43 && data[i + 2] == 0xB6 && data[i + 3] == 0x75 {
            ...
            println!(
                "[DVR] Found Cluster at offset {}, init segment size: {} bytes",
                i, i
            );
...
pub async fn save_dvr_chunk(
    ...
    fs::create_dir_all(&dvr_dir).map_err(|e| format!("Failed to create DVR directory: {}", e))?;
    ...
    if chunk_index == 0 {
        if let Some(init_segment) = extract_init_segment(&data) {
            ...
            println!(
                "[DVR] Saved init segment ({} bytes) to {:?}",
                init_segment.len(),
                init_path
            );
        } else {
    ...
    // Write chunk data
    fs::write(&chunk_path, &data).map_err(|e| format!("Failed to write DVR chunk: {}", e))?;

    let path_str = chunk_path.to_string_lossy().to_string();
    println!(
        "[DVR] Saved chunk {} for {} ({} bytes) to {}",
        chunk_index,
        mint_id,
        data.len(),
        path_str
...
#[tauri::command]
pub async fn cleanup_dvr_chunks(app: AppHandle, mint_id: String) -> Result<(), String> {
    let dvr_dir = get_dvr_dir(&app, &mint_id)?;
...
#[tauri::command]
pub async fn list_dvr_chunks(app: AppHandle, mint_id: String) -> Result<Vec<DvrChunkInfo>, String> {
    let dvr_dir = get_dvr_dir(&app, &mint_id)?;
    ...
    let entries =
        fs::read_dir(&dvr_dir).map_err(|e| format!("Failed to read DVR directory: {}", e))?;
...
#[tauri::command]
pub async fn get_dvr_directory(app: AppHandle, mint_id: String) -> Result<String, String> {
    let dvr_dir = get_dvr_dir(&app, &mint_id)?;
...
#[tauri::command]
pub async fn has_dvr_chunks(app: AppHandle, mint_id: String) -> Result<bool, String> {
    let dvr_dir = get_dvr_dir(&app, &mint_id)?;
...
#[tauri::command]
pub async fn read_all_dvr_chunks(app: AppHandle, mint_id: String) -> Result<Vec<u8>, String> {
    let dvr_dir = get_dvr_dir(&app, &mint_id)?;
    ...
    println!(
        "[DVR] Reading {} chunks for concatenation",
        chunk_files.len()
    );
    ...
    for (index, path) in chunk_files {
        let chunk_data =
            fs::read(&path).map_err(|e| format!("Failed to read chunk {}: {}", index, e))?;
        ...
        println!(
            "[DVR] Added chunk {} ({} bytes), total: {} bytes",
            index,
            chunk_data.len(),
            result.len()
        );
...
pub async fn read_dvr_chunk(
    ...
    let chunk_data =
        fs::read(&chunk_path).map_err(|e| format!("Failed to read DVR chunk: {}", e))?;

    // For chunk 0, return as-is (it already has the init segment)
    if chunk_index == 0 {
        println!(
            "[DVR] Reading chunk 0, returning as-is ({} bytes)",
            chunk_data.len()
        );
        ...
        if chunk0_path.exists() {
            ...
            if let Some(init_segment) = extract_init_segment(&chunk0_data) {
                ...
                fs::write(&init_path, &init_segment)
                    .map_err(|e| format!("Failed to write init segment: {}", e))?;
                println!(
                    "[DVR] Extracted and saved init segment from chunk 0 ({} bytes)",
                    init_segment.len()
                );
                ...
                result.extend_from_slice(&chunk_data);
                println!(
                    "[DVR] Reading chunk {} with prepended init segment ({} bytes total)",
                    chunk_index,
                    result.len()
                );
                return Ok(result);
...
#[tauri::command]
pub async fn read_dvr_init_segment(app: AppHandle, mint_id: String) -> Result<Vec<u8>, String> {
    let dvr_dir = get_dvr_dir(&app, &mint_id)?;
    let init_path = get_init_segment_path(&dvr_dir);
    ...
    let chunk0_data =
        fs::read(&chunk0_path).map_err(|e| format!("Failed to read chunk 0: {}", e))?;
...
pub async fn read_dvr_cluster(
    ...
    if chunk_index == 0 {
        ...
        for i in 0..chunk_data.len().saturating_sub(3) {
                ...
                && chunk_data[i + 3] == 0x75
            {
                ...
                println!(
                    "[DVR] Reading chunk 0 cluster data only ({} bytes, stripped {} bytes init)",
                    cluster_data.len(),
                    i
                );
    ...
    println!(
        "[DVR] Reading chunk {} cluster data ({} bytes)",
        chunk_index,
        chunk_data.len()
    );
    Ok(chunk_data)
...
pub async fn build_vod_from_dvr(
    ...
    for (idx, path) in &chunk_files {
        if *idx == 0 {
            ...
            if let Some(ref init_data) = init_segment {
                let chunk_data =
                    fs::read(path).map_err(|e| format!("Failed to read chunk {}: {}", idx, e))?;
                ...
                println!(
                    "[DVR] Warning: No init segment available for chunk {}, using raw file",
                    idx
                );
                prepared_files.push(path.clone());
            }
    ...
    let mut args = vec![
        "-fflags".to_string(),
        "+genpts+igndts".to_string(), // Regenerate PTS, ignore DTS
        "-f".to_string(),
        "concat".to_string(),
        "-safe".to_string(),
        "0".to_string(),
        ...
        "-vf".to_string(),
        "fps=30,setpts=PTS-STARTPTS".to_string(),
        "-c:v".to_string(),
        encoder.codec.clone(),
    ];
    ...
    args.extend_from_slice(&[
        ...
        "192k".to_string(),
        "-async".to_string(),
        "1".to_string(), // Sync audio to video
        "-pix_fmt".to_string(),
        "yuv420p".to_string(),
        "-movflags".to_string(),
        "+faststart".to_string(),
...
pub async fn build_segment_from_dvr_chunks(
    ...
    if chunk_files.is_empty() {
        return Err(format!(
            "No DVR chunks found in range {}-{} for {}",
            start_chunk, end_chunk, mint_id
        ));
    }
    ...
    println!(
        "[DVR] Building segment {} from {} chunks ({}->{})",
        segment_number,
        chunk_files.len(),
        start_chunk,
        end_chunk
    ...
    if first_chunk_idx != 0 {
        ...
        if let Some(ref init) = init_segment {
            ...
            println!(
                "[DVR] Prepended init segment ({} bytes) for segment starting at chunk {}",
                init.len(),
                first_chunk_idx
            );
        } else {
            return Err(format!(
                "No init segment available for segment starting at chunk {}",
                first_chunk_idx
            ));
        }
    }
    ...
    println!(
        "[DVR] Combined {} chunks into single WebM ({} bytes)",
        chunk_files.len(),
        combined_data.len()
    );
    ...
    let args = vec![
        "-fflags".to_string(),
        "+genpts+igndts".to_string(), // regenerate timestamps
        "-i".to_string(),
        combined_webm_path.to_string_lossy().to_string(),
        "-vf".to_string(),
        "fps=30,setpts=PTS-STARTPTS".to_string(),
        ...
        "30".to_string(), // Force constant 30fps output
        "-g".to_string(),
        "30".to_string(), // Keyframe every 1 second
    ...
    println!(
        "[DVR] Running FFmpeg on combined WebM to build segment from {} chunks...",
        chunk_files.len()
    );
    ...
    if let Ok(metadata) = std::fs::metadata(&output_path) {
        println!(
            "[DVR] Segment {} built successfully: {} ({} MB)",
            segment_number,
            output_path.display(),
        ...
        println!(
            "[DVR] Segment {} built successfully: {}",
            segment_number,
            output_path.display()
        );
    }
