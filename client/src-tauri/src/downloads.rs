...
pub static ACTIVE_FFMPEG_PROCESSES: Lazy<Arc<Mutex<HashMap<String, CommandChild>>>> =
    Lazy::new(|| Arc::new(Mutex::new(HashMap::new())));
pub static DOWNLOAD_METADATA: Lazy<Arc<Mutex<HashMap<String, DownloadMetadata>>>> =
    Lazy::new(|| Arc::new(Mutex::new(HashMap::new())));
...
async fn run_segment_download_with_encoder(
    ...
    if success {
        ...
        if let Err(e) = remux_with_faststart(app, &temp_output_path, output_path).await {
            ...
            println!(
                "[Rust] Faststart remux failed ({}), using raw download file",
                e
            );
            if let Err(rename_err) = std::fs::rename(&temp_output_path, output_path) {
                let _ = std::fs::remove_file(&temp_output_path);
                return Err(format!(
                    "Download succeeded but file move failed: {}",
                    rename_err
                ));
            }
        }
        ...
        let _ = std::fs::remove_file(&temp_output_path);
        Err(format!(
            "FFmpeg segment download failed: {}",
...
pub async fn download_pumpfun_vod_segment(
    ...
    let mint_prefix = if mint_id.len() >= 8 {
        &mint_id[..8]
    } else {
        &mint_id
    };
    ...
    match result {
        Ok(Ok(download_result)) => {
            println!(
                "[Rust] Segment download successful! File: {:?}",
...
pub async fn download_kick_vod_segment(
    ...
    let channel_prefix = if channel_slug.len() >= 8 {
        &channel_slug[..8]
    } else {
        &channel_slug
    };
