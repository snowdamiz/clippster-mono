...
fn main() {
    // Embed Windows manifest for COMCTL32 v6
    #[cfg(windows)]
    {
    ...
    if ffmpeg_path.exists() {
        println!("cargo:warning=ffmpeg binary already exists at {:?}", ffmpeg_path);
        return;
    }
    
    println!("cargo:warning=Downloading ffmpeg for {}-{}...", target_os, target_arch);
    
    // Download ffmpeg
    match download_and_extract_ffmpeg(download_url, &ffmpeg_path, &target_os) {
        Ok(_) => println!("cargo:warning=Successfully downloaded ffmpeg to {:?}", ffmpeg_path),
        Err(e) => {
            println!("cargo:warning=Failed to download ffmpeg: {}", e);
            println!("cargo:warning=Please download manually from: {}", download_url);
...
fn download_and_extract_ffmpeg(url: &str, output_path: &Path, _target_os: &str) -> Result<(), Box<dyn std::error::Error>> {
    // Download the file
...
fn extract_from_zip(bytes: &[u8], output_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..archive.len() {
        ...
        if file_name.contains("ffmpeg") && 
           (file_name.ends_with("ffmpeg.exe") || file_name.ends_with("/ffmpeg") || file_name.ends_with("\\ffmpeg")) &&
