          ...
          <div v-if="open" class="instagram-dialog" role="dialog" aria-modal="true">
            ...
            <div class="instagram-dialog__content">
              <form @submit.prevent="publish" class="instagram-dialog__form">
                ...
                <!-- Instagram Account -->
                <div class="instagram-dialog__field">
                  <label class="instagram-dialog__label">Instagram Account *</label>
                  <div class="relative">
                    <button
                      type="button"
                      @click="showAccountDropdown = !showAccountDropdown"
                      class="instagram-dialog__input instagram-dialog__dropdown-trigger"
                    ...
                    <div v-if="showAccountDropdown" class="instagram-dialog__dropdown">
                      <template v-if="orgAccounts.length > 0">
                        <div class="instagram-dialog__dropdown-group">{{ currentOrgName }} Accounts</div>
                        <button
                          v-for="account in orgAccounts"
                          :key="`org-${account.id}`"
                          type="button"
                          @click="selectAccount(`org:${account.id}`, `@${account.username}`)"
                          class="instagram-dialog__dropdown-item"
                          :class="{ 'instagram-dialog__dropdown-item--selected': selectedAccountValue === `org:${account.id}` }"
                        >
                          @{{ account.username }}
                        </button>
                      </template>
                      <template v-if="showPersonalAccounts && personalAccounts.length > 0">
                        <div class="instagram-dialog__dropdown-group">My Personal Accounts</div>
                        <button
                          ...
                          class="instagram-dialog__dropdown-item"
                          :class="{ 'instagram-dialog__dropdown-item--selected': selectedAccountValue === `user:${account.id}` }"
                        >
                          @{{ account.username }}
                        </button>
                      </template>
                ...
                <div v-if="creatorProfiles.length > 0" class="instagram-dialog__field">
                  <label class="instagram-dialog__label">
                    Creator Profile
                  ...
                  <div class="relative">
                    ...
                    <div v-if="showCreatorDropdown" class="instagram-dialog__dropdown">
                      <button
                        type="button"
                        @click="selectCreator('', 'None')"
                        class="instagram-dialog__dropdown-item"
                        :class="{ 'instagram-dialog__dropdown-item--selected': selectedCreatorProfileId === '' }"
                      >
                        None
                      </button>
                      <button
                        ...
                        class="instagram-dialog__dropdown-item"
                        :class="{ 'instagram-dialog__dropdown-item--selected': selectedCreatorProfileId === String(profile.id) }"
                      >
                        {{ profile.name }}
                      </button>
                    </div>
...
<style scoped>
  ...
  /* ===== Custom Dropdown ===== */
  .instagram-dialog__dropdown-trigger {
    display: flex;
    align-items: center;
  ...
  .instagram-dialog__dropdown-trigger:hover {
    border-color: rgba(255, 255, 255, 0.1);
  }

  .instagram-dialog__dropdown-trigger:disabled {
    opacity: 0.5;
    cursor: not-allowed;
  }

  .instagram-dialog__dropdown {
    position: absolute;
    top: calc(100% + 0.5rem);
    left: 0;
    right: 0;
  ...
  .instagram-dialog__dropdown::-webkit-scrollbar {
    width: 6px;
  }

  .instagram-dialog__dropdown::-webkit-scrollbar-track {
    background: transparent;
  }

  .instagram-dialog__dropdown::-webkit-scrollbar-thumb {
    background-color: rgba(255, 255, 255, 0.15);
    border-radius: 3px;
  }

  .instagram-dialog__dropdown-group {
  ...
  .instagram-dialog__dropdown-item {
    display: block;
    width: 100%;
    text-align: left;
  ...
  .instagram-dialog__dropdown-item:hover {
    background-color: var(--sidebar-hover);
  }

  .instagram-dialog__dropdown-item--selected {
    background-color: rgba(6, 182, 212, 0.15);
    color: var(--sidebar-accent);
  }
