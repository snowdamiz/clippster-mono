<template>
  <div v-if="modelValue" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm">
    <div class="bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl w-[900px] max-w-[95vw] max-h-[95vh] flex flex-col overflow-hidden">
      <!-- Header -->
        ...
        >
          ...
          <div class="absolute inset-0 pointer-events-none">
            ...
            <div class="absolute inset-0 bg-black/60" 
                 style="clip-path: polygon(0% 0%, 0% 100%, 100% 100%, 100% 0%, 0% 0%, var(--roi-left) var(--roi-top), var(--roi-right) var(--roi-top), var(--roi-right) var(--roi-bottom), var(--roi-left) var(--roi-bottom), var(--roi-left) var(--roi-top))"
                 :style="{
                   '--roi-left': roi.x + '%',
                   '--roi-top': roi.y + '%',
          ...
          >
            ...
            <!-- Handles (Corners) -->
            <div class="absolute -top-1.5 -left-1.5 w-3 h-3 bg-white border border-violet-500 rounded-full cursor-nw-resize z-10 opacity-0 group-hover:opacity-100 transition-opacity" @mousedown.stop="startResize($event, 'nw')"></div>
            <div class="absolute -top-1.5 -right-1.5 w-3 h-3 bg-white border border-violet-500 rounded-full cursor-ne-resize z-10 opacity-0 group-hover:opacity-100 transition-opacity" @mousedown.stop="startResize($event, 'ne')"></div>
            <div class="absolute -bottom-1.5 -left-1.5 w-3 h-3 bg-white border border-violet-500 rounded-full cursor-sw-resize z-10 opacity-0 group-hover:opacity-100 transition-opacity" @mousedown.stop="startResize($event, 'sw')"></div>
            <div class="absolute -bottom-1.5 -right-1.5 w-3 h-3 bg-white border border-violet-500 rounded-full cursor-se-resize z-10 opacity-0 group-hover:opacity-100 transition-opacity" @mousedown.stop="startResize($event, 'se')"></div>
          </div>
        </div>
...
function onMove(e: MouseEvent) {
  ...
  if (isDragging) {
    ...
    if (newX + newWidth > 100) {
      ...
      if (!isDragging) { // Only checking during resize
         const maxW = 100 - newX;
         if (newWidth > maxW) {
           newWidth = maxW;
           newHeight = newWidth / pctRatio;
