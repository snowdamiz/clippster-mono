<template>
  ...
  >
    <!-- Track Header/Label -->
    <div class="track-label w-[100px] h-full pl-2 pr-2 flex flex-col justify-center text-[11px] sticky left-0 z-[70] bg-[#0c0c0c] flex-shrink-0 border-r border-white/[0.08]">
      <div class="flex items-center gap-1 text-white/50 mb-1 opacity-0 group-hover/track:opacity-100 transition-opacity">
        <button 
    ...
    >
      ...
      <div
        ...
        :class="[
          getItemClasses(item),
          { 'ring-2 ring-white/50 z-10': isSelected(item.id) }
        ]"
      ...
      >
        ...
        <!-- Keyframe Indicators -->
        <div v-if="item.keyframes && item.keyframes.length > 0" class="absolute inset-x-0 bottom-0 h-3 pointer-events-none">
          <div
        ...
        <div class="resize-handle-l absolute left-0 top-0 bottom-0 w-2 cursor-ew-resize opacity-0 group-hover/item:opacity-100 hover:bg-white/20 z-20"
             @mousedown.stop="(e) => $emit('resizeStart', e, item, 'left')"></div>
        <div class="resize-handle-r absolute right-0 top-0 bottom-0 w-2 cursor-ew-resize opacity-0 group-hover/item:opacity-100 hover:bg-white/20 z-20"
             @mousedown.stop="(e) => $emit('resizeStart', e, item, 'right')"></div>
      </div>
    </div>
  </div>
</template>
...
function getItemColorClass(item: TimelineItem) {
  switch (item.type) {
    ...
    case 'sticker': return 'bg-pink-500';
