          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                @click="saveVideoToLocation(build.file_path, `${props.clipName || 'video'}.${build.output_format || 'mp4'}`)"
...
<script setup lang="ts">
  ...
  async function setupEventListeners() {
    ...
    unlistenProgress = await listen<{ clipId: string; clip_id?: string; progress: number }>('clip-build-progress', (event) => {
    ...
    unlistenComplete = await listen<{ clipId: string; clip_id?: string; buildId?: string; build_id?: string }>('clip-build-complete', (event) => {
