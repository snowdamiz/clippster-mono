  ...
  <div class="track-renderer absolute inset-0 pointer-events-none">
    ...
    >
      ...
      >
        ...
        <!-- Chroma Key effect -->
        <div
          v-else-if="getEffectType(item) === 'chroma'"
          class="absolute inset-0"
          :style="getChromaKeyStyle(item)"
        />
      </div>
      ...
      >
        ...
        >
          <span class="text-purple-400 text-xs bg-black/50 px-2 py-1 rounded">
            Adjustment Layer
          </span>
        </div>
      </div>
...
const visibleItems = computed(() => {
  ...
  return props.track.items.filter(item => {
    return props.currentTime >= item.startTime && 
           props.currentTime < (item.startTime + item.duration);
  });
});
...
function isEmoji(item: TimelineItem): boolean {
  return item.type === 'sticker' && (item.originalData?.stickerType === 'emoji' || item.originalData?.sticker_type === 'emoji');
}
...
function getFilterString(settings: any): string {
  ...
  if (settings.brightness !== undefined && settings.brightness !== 0) filters.push(`brightness(${100 + settings.brightness}%)`);
  if (settings.contrast !== undefined && settings.contrast !== 0) filters.push(`contrast(${100 + settings.contrast}%)`);
  if (settings.saturation !== undefined && settings.saturation !== 0) filters.push(`saturate(${100 + settings.saturation}%)`);
