...
async function seekAndBuffer(video: HTMLVideoElement, targetTime: number, slot: Slot): Promise<void> {
  ...
  if (video.readyState >= 3) {
    console.log(`[VideoCompositor] Slot ${slot} already has enough data (readyState: ${video.readyState})`);
  ...
  await new Promise<void>((resolve) => {
    const onCanPlay = () => {
      ...
      console.log(`[VideoCompositor] Slot ${slot} buffered and ready at ${video.currentTime}`);
      slotReady.value[slot] = true;
      resolve();
    };
    video.addEventListener('canplaythrough', onCanPlay);
    
    // Timeout fallback - mark ready anyway after 2 seconds
    setTimeout(() => {
      video.removeEventListener('canplaythrough', onCanPlay);
...
async function updatePlayback(): Promise<void> {
  ...
  if (slotSourceIds.value[inactiveSlot] === source.id && slotReady.value[inactiveSlot]) {
    ...
    if (newVideo) {
      ...
      if (props.isPlaying && newVideo.readyState >= 2) {
...
function onVideoCanPlay(slot: Slot): void {
  ...
  console.log(`[VideoCompositor] Video canplaythrough in slot ${slot}, readyState: ${readyState}, currentTime: ${video?.currentTime}`);
  
  // Only mark as ready if we have enough data (readyState >= 3 means HAVE_FUTURE_DATA)
  if (readyState < 3) {
    console.log(`[VideoCompositor] Slot ${slot} not ready enough yet (readyState ${readyState})`);
    return;
  }
  ...
  if (source && slotSourceIds.value[slot] === source.id && slot !== activeSlot.value) {
    console.log(`[VideoCompositor] Swapping to newly ready slot ${slot}`);
...
watch(
  ...
  (playing) => {
    ...
    if (playing && video.paused && video.readyState >= 2 && !props.isInGap) {
      video.play().catch(() => {});
