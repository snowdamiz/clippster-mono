  ...
  <Dialog v-model:open="isOpen">
    <DialogContent class="max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
      <DialogHeader>
        ...
        <DialogDescription v-if="campaign?.organization">
      ...
      <div v-else-if="campaignDetails" class="flex-1 overflow-y-auto space-y-6 pr-2">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div class="bg-muted/30 rounded-lg p-3 text-center">
            <div class="text-lg font-bold text-foreground">{{ campaignDetails.min_views_for_payment.toLocaleString() }}</div>
        ...
        <div v-if="participation" class="bg-green-500/10 rounded-lg p-4 border border-green-500/20">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-medium text-green-500">
              {{ participation.status === 'approved' ? 'You are a member of this campaign' : 
                 participation.status === 'pending' ? 'Your application is pending' : 
  ...
  <Dialog v-model:open="showApplicationForm">
    <DialogContent class="max-w-md">
      <DialogHeader>
        ...
        <DialogDescription>
          Tell the campaign owner why you'd like to join
        </DialogDescription>
      </DialogHeader>
  ...
  <Dialog v-model:open="showSubmitForm">
    <DialogContent class="max-w-md">
      <DialogHeader>
        ...
        <DialogDescription>
          Paste the URL of your posted clip
        </DialogDescription>
      </DialogHeader>
...
import { 
  Megaphone, Loader2, UserPlus, ClipboardCheck, CheckCircle, Upload, Wallet,
...
import { 
  getCampaign, applyToCampaign, submitClip, 
  type Campaign, type CampaignParticipation, type CampaignStats,
  getPlatformDisplayName, detectPlatformFromUrl
...
const submitClipToServer = async () => {
  ...
  try {
    const response = await submitClip(
      ...
      clipUrl.value,
      detectedPlatform.value || undefined
    );
    if (response.success) {
      toast({
