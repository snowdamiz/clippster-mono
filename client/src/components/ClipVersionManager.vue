<template>
  <div v-if="showVersions" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
    <div class="bg-card rounded-lg p-4 max-w-2xl max-h-[80vh] overflow-auto border border-border shadow-2xl w-full mx-4">
      <div class="flex items-center justify-between mb-4">
        ...
        <button @click="closeVersions" class="p-1 hover:bg-muted/50 rounded-md">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-foreground/70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-2 mb-2">
                <span
                  ...
                  :class="getChangeTypeClass(version.change_type)"
                >
      ...
      <div v-if="showComparison && comparingVersion" class="mt-4 pt-4 border-t border-border">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div class="text-xs space-y-1">
              <div><strong>Name:</strong> {{ comparingVersion.name }}</div>
              <div><strong>Duration:</strong> {{ formatDuration(comparingVersion.end_time - comparingVersion.start_time) }}</div>
              <div><strong>Timing:</strong> {{ Math.floor(comparingVersion.start_time) }}s - {{ Math.floor(comparingVersion.end_time) }}s</div>
            </div>
          </div>
        </div>
      </div>
...
import { ref } from 'vue'
import { getClipVersionsByClipId, getCurrentClipVersion, restoreClipVersion, type ClipVersion } from '@/services/database'
import {
  HistoryIcon,
  TrendingUpIcon,
  RotateCcwIcon,
...
watch(() => props.showVersions, async (show) => {
  if (show) {
    await loadVersions()
  }
})
