  ...
  <div class="group relative bg-card border border-border rounded-lg overflow-hidden hover:border-foreground/20 cursor-pointer">
    ...
    <div class="aspect-video bg-muted/50 relative">
      ...
      <div class="relative z-10 h-full flex flex-col">
        <!-- Duration Badge -->
        <div v-if="clip.duration" class="absolute bottom-2 right-2 px-2 py-1 bg-black/60 text-white text-xs rounded-md backdrop-blur-sm">
          {{ formattedDuration }}
        </div>
        ...
        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 transition-opacity">
          <button class="p-2.5 bg-white/90 hover:bg-white rounded-lg transition-colors" title="Download" @click.stop="downloadClip">
