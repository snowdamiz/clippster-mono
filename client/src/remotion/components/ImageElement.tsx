...
export const ImageElement: React.FC<Props> = ({ track, trackTime, trackDuration, videoServerPort }) => {
  ...
  const width = getAnimatedValue(props.width, trackTime, trackDuration, 30) || (typeof props.width === 'number' ? props.width : 1920);
  const height = getAnimatedValue(props.height, trackTime, trackDuration, 30) || (typeof props.height === 'number' ? props.height : 1080);
  ...
  const effectStyle = applyEffects(props.effects || [], trackTime, trackDuration, 30);

  if (!track.source) return null;

  const encodedPath = utf8ToBase64(track.source.path);
  const imageUrl = `http://localhost:${videoServerPort}/video/${encodedPath}`;
