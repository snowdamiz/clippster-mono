  ...
  try {
    ...
    if (streamer.platform === 'Kick') {
      ...
      if (state && state.accumulatedChunks > 0) {
        console.log(`[LiveMonitor] Processing ${state.accumulatedChunks} remaining chunks for final segment`);
        ...
        if (dvrSession && dvrSession.chunks.length > 0) {
          ...
          try {
            ...
            const actualDuration = (lastChunk.index - state.segmentStartChunk + 1) * DVR_CHUNK_DURATION;
    ...
    try {
      ...
      if (streamer.platform === 'Kick') {
        ...
        const onChunkReady = async (
          ...
          console.log(`[LiveMonitor] DVR chunk ${chunk.index} ready, accumulated: ${state.accumulatedChunks}/${state.chunksPerSegment}`);
          ...
          if (state.accumulatedChunks >= state.chunksPerSegment) {
            ...
            console.log(`[LiveMonitor] Building segment ${segmentNum} from chunks ${startChunk}-${endChunk}`);
        ...
        await dvrRecording.startDvrSession(
          streamer.mintId,
          streamer.id,
          streamer.displayName,
          {
