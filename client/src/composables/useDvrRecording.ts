...
async function checkLiveStatus(mintId: string): Promise<{ isLive: boolean; token?: string; serverUrl?: string }> {
  try {
...
async function joinLivestream(mintId: string): Promise<{ token: string; serverUrl: string }> {
  try {
    ...
    let serverUrl = data.serverUrl || data.url || data.wsUrl || 'wss://pump-prod-tg2x8veh.livekit.cloud';
...
function waitForTracks(room: Room): Promise<CaptureSetup> {
  return new Promise((resolve, reject) => {
        ...
        videoElement.play().then(() => {
          ...
          const waitForDimensions = () => {
            if (videoElement.videoWidth > 0 && videoElement.videoHeight > 0) {
              setupCanvasCapture(videoElement, audioTrackRef!, resolve);
            } else {
    ...
    function setupCanvasCapture(
      ...
      resolve: (result: CaptureSetup) => void
    ) {
      ...
      const drawFrame = () => {
        ...
        if (now - lastLogTime >= 5000) {
          console.log(`[DvrRecording] Canvas drawing: ${frameCount} frames in 5s (${(frameCount / 5).toFixed(1)} fps), video readyState: ${videoElement.readyState}`);
          frameCount = 0;
          lastLogTime = now;
        }
      ...
      if (audioMST) {
        ...
        canvasStream.getVideoTracks().forEach(track => {
          console.log(`[DvrRecording] Adding canvas video track: enabled=${track.enabled}, readyState=${track.readyState}`);
          combinedStream.addTrack(track);
        });
        
        // Add audio track directly (no need to go through AudioContext for recording)
        console.log(`[DvrRecording] Adding audio track: enabled=${audioMST.enabled}, readyState=${audioMST.readyState}, muted=${audioMST.muted}`);
        combinedStream.addTrack(audioMST);
        
        console.log('[DvrRecording] Combined stream ready:', combinedStream.getTracks().length, 'tracks');
        combinedStream.getTracks().forEach(t => {
          console.log(`[DvrRecording] Combined track: ${t.kind}, enabled=${t.enabled}, readyState=${t.readyState}, muted=${t.muted}`);
        });
    ...
    const handleTrack = (track: RemoteTrack, publication: RemoteTrackPublication, participant: RemoteParticipant) => {
      console.log('[DvrRecording] Track received:', track.kind, 'from participant:', participant.identity);
    ...
    timeout = window.setTimeout(() => {
      ...
      if (hasVideo || hasAudio) {
        console.warn('[DvrRecording] Timeout waiting for all tracks, proceeding with available tracks');
          ...
          const checkVideo = () => {
            ...
            if (videoElement.readyState >= 2 && videoElement.videoWidth > 0) {
              ...
              if (videoTrackRef && audioTrackRef) {
                ...
                reject(new Error('Video never got data'));
              }
            } else {
              // Keep waiting
        ...
        reject(new Error('Timeout waiting for media tracks'));
      }
    }, 30000);
  });
}
...
export function useDvrRecording() {
  ...
  const getDvrSessionState = (mintId: string) => computed(() => activeDvrSessions.value.get(mintId));
  ...
  async function startDvrSession(
    ...
    try {
      ...
      try {
        ...
        console.log('[DvrRecording] MediaStream ready, tracks:', captureSetup.mediaStream.getTracks().length);
        captureSetup.mediaStream.getTracks().forEach(track => {
          console.log(`[DvrRecording] Track: ${track.kind}, enabled: ${track.enabled}, readyState: ${track.readyState}, muted: ${track.muted}`);
        });
  ...
  async function startMediaRecorder(
    ...
    recorder.ondataavailable = async (event) => {
      ...
      try {
        ...
        console.log(`[DvrRecording] Saving chunk ${currentSession.chunkIndex} (${data.length} bytes)...`);
        
        // Save chunk via Tauri
        const chunkPath = await invoke<string>('save_dvr_chunk', {
          mintId,
          chunkIndex: currentSession.chunkIndex,
          data,
        });
        
        // Calculate timing
        const startTime = currentSession.totalDuration;
        const duration = 4; // 4-second chunks
        
        const chunk: DvrChunk = {
          index: currentSession.chunkIndex,
          path: chunkPath,
          duration,
          startTime,
        ...
        console.log(`[DvrRecording] Chunk ${chunk.index} saved for ${mintId}, total duration: ${currentSession.totalDuration}s, path: ${chunkPath}`);
    ...
    const statusInterval = setInterval(() => {
      ...
      console.log(`[DvrRecording] Status for ${mintId}: recorder=${recorder.state}, stream.active=${mediaStream.active}, chunks=${currentSession.chunks.length}, duration=${currentSession.totalDuration}s`);
