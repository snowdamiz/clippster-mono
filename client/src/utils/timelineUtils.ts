...
// Helper to extract word text from different possible property names
function getWordText(wordObj: any): string | null {
  // Check various property names that might contain the word text
  const text = wordObj.word || wordObj.text || wordObj.content;
  return typeof text === 'string' && text.trim() ? text.trim() : null;
}

// Helper to extract a single word with timing from an object
function extractWord(wordObj: any): WordInfo | null {
  const wordText = getWordText(wordObj);
  const start = wordObj.start ?? wordObj.startTime ?? wordObj.begin;
  const end = wordObj.end ?? wordObj.endTime ?? wordObj.finish;
  ...
  if (wordText && typeof start === 'number' && typeof end === 'number') {
    return {
      ...
      confidence: wordObj.confidence ?? wordObj.prob ?? wordObj.probability,
    };
  }
  return null;
}
...
export function parseTranscriptToWords(rawJson: string): WordInfo[] {
  try {
    ...
    console.log('[parseTranscriptToWords] Parsing transcript, data structure:', {
      hasWords: !!data.words,
    ...
    if (data.words && Array.isArray(data.words)) {
      ...
      console.log(
        '[parseTranscriptToWords] Found top-level words array, count:',
        data.words.length
      );
    ...
    if (Array.isArray(data)) {
      ...
      console.log('[parseTranscriptToWords] Data is direct array, count:', data.length);
      data.forEach((wordObj: any) => {
        const word = extractWord(wordObj);
        if (word) words.push(word);
      });
    }
    ...
    console.log(
      ...
      uniqueWords.length > 0
        ? `(first: "${uniqueWords[0]?.word}", last: "${uniqueWords[uniqueWords.length - 1]?.word}")`
        : ''
    );
