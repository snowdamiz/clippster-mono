  ...
  <div class="clipper-profile-page">
    ...
    >
      ...
      <div class="group relative mb-8">
        <div class="absolute inset-0 bg-gradient-to-br from-violet-500/10 to-transparent rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        <div class="relative bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-2xl overflow-hidden hover:border-border transition-all duration-300">
          <!-- Gradient Header Bar -->
          ...
          <div class="p-6 md:p-8">
            <div class="flex flex-col md:flex-row items-start gap-6">
              <!-- Avatar -->
              <div class="relative">
                <div class="w-24 h-24 md:w-28 md:h-28 rounded-2xl bg-gradient-to-br from-violet-500/20 to-indigo-500/20 flex items-center justify-center overflow-hidden flex-shrink-0 border-2 border-violet-500/30 shadow-lg shadow-violet-500/10">
                ...
                <div v-if="clipperProfile?.is_verified" class="absolute -bottom-1 -right-1 p-1.5 bg-blue-500 rounded-full border-2 border-card">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-3 mb-2">
                  ...
                  <Badge v-if="clipperProfile?.looking_for_work" class="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-xs font-semibold px-2.5 py-1">
                ...
                <div class="flex flex-wrap items-center gap-4 md:gap-6">
                  <div class="flex items-center gap-2 px-3 py-2 bg-muted/30 rounded-lg border border-border/30">
                    ...
                    <span class="text-sm"><strong class="text-foreground font-semibold">{{ clipperProfile?.total_campaigns_completed || 0 }}</strong> <span class="text-muted-foreground">campaigns</span></span>
                  </div>
                  <div class="flex items-center gap-2 px-3 py-2 bg-muted/30 rounded-lg border border-border/30">
                    <Film class="w-4 h-4 text-indigo-400" />
                    <span class="text-sm"><strong class="text-foreground font-semibold">{{ clipperProfile?.total_clips_delivered || 0 }}</strong> <span class="text-muted-foreground">clips</span></span>
                  </div>
                  <div class="flex items-center gap-2 px-3 py-2 bg-muted/30 rounded-lg border border-border/30">
                    <Star class="w-4 h-4 text-amber-400" />
                    <span class="text-sm"><strong class="text-foreground font-semibold">{{ clipperProfile?.total_endorsements || 0 }}</strong> <span class="text-muted-foreground">endorsements</span></span>
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-row md:flex-col gap-2 flex-shrink-0">
                ...
                <div v-if="clipperProfile && !clipperProfile.is_public" class="flex items-center gap-1.5 text-xs text-muted-foreground px-3 py-1.5 bg-muted/30 rounded-lg">
      ...
      <Tabs v-model="activeTab">
        ...
        <TabsContent value="leaderboard" class="space-y-6">
            <!-- TODO Banner -->
            <div class="bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 rounded-xl p-4 flex items-center gap-3">
              <div class="p-2 bg-amber-500/20 rounded-lg">
                <AlertTriangle class="w-5 h-5 text-amber-400" />
              </div>
              <div>
                <p class="text-sm font-semibold text-amber-400">Leaderboard In Progress</p>
                <p class="text-xs text-muted-foreground">View tracking not yet implemented. See <code class="bg-muted/50 px-1.5 py-0.5 rounded text-amber-400/80">docs/Leaderboard_TODO.md</code> for remaining tasks.</p>
              </div>
            </div>

            <!-- Your Ranking Card -->
            <div class="group relative">
              <div class="absolute inset-0 bg-gradient-to-br from-amber-500/10 to-transparent rounded-xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <div class="relative bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-xl overflow-hidden hover:border-border transition-all duration-300">
                <div class="h-1 w-full bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-500" />
                <div class="p-6">
                  <div class="flex items-center gap-3 mb-6">
                    <div class="p-2.5 bg-gradient-to-br from-amber-500/20 to-yellow-500/20 rounded-xl border border-amber-500/30">
                  ...
                  <div class="grid grid-cols-3 gap-4">
                    <div class="text-center p-4 bg-muted/20 rounded-xl border border-border/30">
                      <div class="text-3xl font-bold bg-gradient-to-r from-amber-400 to-yellow-400 bg-clip-text text-transparent">{{ myRank || '--' }}</div>
            ...
            <div class="bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-xl overflow-hidden">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex gap-1 bg-muted/30 rounded-lg p-1 border border-border/30">
                    <button
                      ...
                      :class="leaderboardPeriod === 'weekly' ? 'bg-background text-foreground shadow-sm border border-border/50' : 'text-muted-foreground hover:text-foreground'"
                    >
                      Weekly
                    </button>
                    <button
                      ...
                      class="px-3 py-1.5 text-xs font-medium rounded-md transition-all"
                      :class="leaderboardPeriod === 'monthly' ? 'bg-background text-foreground shadow-sm border border-border/50' : 'text-muted-foreground hover:text-foreground'"
                    >
                      Monthly
                    </button>
                  </div>
              ...
              <div class="p-4">
                ...
                <div v-else class="space-y-2">
                  <div
                    ...
                    :class="entry.clipper_profile?.user_id === currentUserId ? 'bg-violet-500/10 border border-violet-500/30 shadow-sm' : 'bg-muted/20 hover:bg-muted/30 border border-transparent'"
                  >
                    <!-- Rank -->
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-sm shadow-sm"
                      :class="{
                    ...
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                      <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500/20 to-indigo-500/20 flex items-center justify-center overflow-hidden flex-shrink-0 border border-violet-500/20">
                      ...
                      <div class="min-w-0">
                        <div class="font-medium text-foreground truncate text-sm">
                          ...
                          <span v-if="entry.clipper_profile?.user_id === currentUserId" class="text-xs text-violet-400 ml-1 font-semibold">(You)</span>
                        </div>
                        <div class="text-xs text-muted-foreground">
                          {{ entry.clips_delivered }} clips delivered
                        </div>
                      </div>
                    </div>
          ...
          <div class="max-w-3xl">
            <div class="bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-xl overflow-hidden">
              <div class="p-6 border-b border-border/50">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="p-2.5 bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-xl border border-blue-500/30">
                  ...
                  <Button size="sm" @click="openAddSocialAccount" class="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white shadow-lg shadow-blue-500/20">
              ...
              <div class="p-4">
                ...
                <div v-else-if="socialAccounts.length === 0" class="text-center py-12">
                  ...
                  <Button size="sm" @click="openAddSocialAccount" variant="outline" class="hover:border-blue-500/50 hover:bg-blue-500/5">
                    <Plus class="w-4 h-4 mr-1.5" />
                ...
                <div v-else class="space-y-3">
                  ...
                  >
                    <div class="flex items-center gap-4">
                      ...
                      >
                        <component :is="getPlatformIcon(account.platform)" class="w-6 h-6" :class="getPlatformIconClass(account.platform)" />
                      </div>
                      <div>
                    ...
                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" @click="editSocialAccount(account)" class="h-8 w-8 hover:bg-background">
                        <Pencil class="w-4 h-4 text-muted-foreground" />
                      </Button>
                      <Button variant="ghost" size="icon" @click="confirmDeleteSocialAccount(account)" class="h-8 w-8 hover:bg-red-500/10">
          ...
          <div class="max-w-3xl">
            <div class="bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-xl overflow-hidden">
              <div class="p-6 border-b border-border/50">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="p-2.5 bg-gradient-to-br from-emerald-500/20 to-green-500/20 rounded-xl border border-emerald-500/30">
                  ...
                  <Button size="sm" @click="openAddPaymentMethod" class="bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 text-white shadow-lg shadow-emerald-500/20">
                    <Plus class="w-4 h-4 mr-1.5" />
                    Add Method
                  </Button>
                </div>
              ...
              <div class="p-4">
                ...
                <div v-else-if="paymentMethods.length === 0" class="text-center py-12">
                  ...
                  <Button size="sm" @click="openAddPaymentMethod" variant="outline" class="hover:border-emerald-500/50 hover:bg-emerald-500/5">
                    <Plus class="w-4 h-4 mr-1.5" />
                ...
                <div v-else class="space-y-3">
                  <div
                    ...
                    :class="method.is_default ? 'bg-emerald-500/5 border-emerald-500/30' : 'bg-muted/20 hover:bg-muted/30 border-transparent hover:border-border/50'"
                  >
                    <div class="flex items-center gap-4">
                      <div class="w-12 h-12 rounded-xl flex items-center justify-center"
                        :class="getPaymentMethodBgClass(method.method_type)"
                      >
                        <component :is="getPaymentMethodIcon(method.method_type)" class="w-6 h-6" :class="getPaymentMethodIconClass(method.method_type)" />
                      </div>
                      <div>
                        <div class="flex items-center gap-2">
                          <span class="font-semibold text-foreground">
                          ...
                          <Badge v-if="method.is_default" class="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-[10px] font-semibold">Default</Badge>
                        </div>
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button variant="ghost" size="icon" @click="editPaymentMethod(method)" class="h-8 w-8 hover:bg-background">
                          <Pencil class="w-4 h-4 text-muted-foreground" />
                        </Button>
                        <Button variant="ghost" size="icon" @click="confirmDeletePaymentMethod(method)" class="h-8 w-8 hover:bg-red-500/10">
                          <Trash2 class="w-4 h-4 text-red-400" />
                        </Button>
                      </div>
                    </div>
        ...
        <TabsContent value="campaigns" class="space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div class="group relative">
                <div class="absolute inset-0 bg-gradient-to-br from-emerald-500/10 to-transparent rounded-xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div class="relative bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-xl p-4 hover:border-emerald-500/30 transition-all">
                  <div class="flex items-center gap-2 mb-2">
                    <div class="p-1.5 bg-emerald-500/20 rounded-lg">
                      <DollarSign class="w-4 h-4 text-emerald-400" />
                    </div>
                  </div>
                  <div class="text-2xl font-bold bg-gradient-to-r from-emerald-400 to-green-400 bg-clip-text text-transparent">${{ formatAmount(earningsSummary.total_earned) }}</div>
                  <div class="text-xs text-muted-foreground mt-1 font-medium">Total Earned</div>
                </div>
              </div>
              <div class="group relative">
                <div class="absolute inset-0 bg-gradient-to-br from-amber-500/10 to-transparent rounded-xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div class="relative bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-xl p-4 hover:border-amber-500/30 transition-all">
                  <div class="flex items-center gap-2 mb-2">
                    <div class="p-1.5 bg-amber-500/20 rounded-lg">
                      <Clock class="w-4 h-4 text-amber-400" />
                    </div>
                  </div>
                  <div class="text-2xl font-bold bg-gradient-to-r from-amber-400 to-yellow-400 bg-clip-text text-transparent">${{ formatAmount(earningsSummary.pending) }}</div>
                  <div class="text-xs text-muted-foreground mt-1 font-medium">Pending</div>
                </div>
              </div>
              <div class="bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-xl p-4 hover:border-border transition-all">
                <div class="flex items-center gap-2 mb-2">
            ...
            <div class="bg-gradient-to-br from-card via-card to-muted/20 border border-border/80 rounded-xl overflow-hidden">
              <div class="p-6 border-b border-border/50">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="p-2 bg-gradient-to-br from-violet-500/20 to-indigo-500/20 rounded-lg border border-violet-500/30">
                      <Megaphone class="h-4 w-4 text-violet-400" />
                    </div>
                    <h3 class="font-semibold text-foreground">Campaign History</h3>
                  </div>
                  <Button size="sm" variant="outline" @click="$router.push('/campaigns')" class="hover:border-violet-500/50 hover:bg-violet-500/5">
                    Browse Campaigns
                  </Button>
                </div>
              </div>
              ...
              <div class="p-4">
                ...
                <div v-else-if="myCampaigns.length === 0" class="text-center py-12">
                  ...
                  <Button @click="$router.push('/campaigns')" class="bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white shadow-lg shadow-violet-500/20">
                ...
                <div v-else class="space-y-3">
                  ...
                  >
                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-violet-500/20 to-indigo-500/20 flex items-center justify-center flex-shrink-0 border border-violet-500/20">
                      <Megaphone class="w-6 h-6 text-violet-400" />
                    </div>
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center justify-between gap-2">
                        <h4 class="font-semibold text-sm text-foreground truncate group-hover:text-violet-400 transition-colors">{{ campaign.title }}</h4>
                  ...
                  >
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2 mb-1">
                        <div class="w-6 h-6 rounded-md flex items-center justify-center" :class="getPlatformBgClass(submission.platform)">
                          <component :is="getPlatformIcon(submission.platform)" class="w-3.5 h-3.5" :class="getPlatformIconClass(submission.platform)" />
                        </div>
                        <a 
...
import { 
  ...
  Megaphone, Calendar, Eye, EyeOff, Upload, Trophy, AlertTriangle, Film, Star, Clock, Users
