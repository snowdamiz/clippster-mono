...
async function loadAudioFiles() {
	try {
		...
		console.log('[XSpacesView] All audio files loaded:', audioFiles.value.length);
		console.log('[XSpacesView] Audio files with platforms:', audioFiles.value.map(a => ({ title: a.title, platform: a.platform })));
		console.log('[XSpacesView] Filtered Twitter Spaces:', audioFiles.value.filter(a => a.platform === 'Twitter').length);
	} catch (error) {
...
<template>
	<div class="flex h-full flex-col bg-transparent">
		...
		<div v-else-if="filteredAudio.length === 0" class="flex-1 flex flex-col items-center justify-center gap-2 px-4 text-center">
			...
			<p class="text-sm text-zinc-400">No X Spaces found</p>
			<p class="text-xs text-zinc-500">Download X Spaces from the Audio Library page</p>
		</div>
