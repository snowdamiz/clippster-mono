	...
	<div class="flex h-full flex-col overflow-hidden">
		...
		<div class="border-b border-white/10 px-4 py-2.5">
			<div class="flex items-center gap-2">
				<span class="text-sm font-medium text-zinc-200">Branding</span>
				<span
					v-if="configuredRatioCount > 0"
					class="rounded-full bg-blue-500/20 px-1.5 py-0.5 text-[10px] font-medium text-blue-400"
				>
					{{ configuredRatioCount }} ratio{{ configuredRatioCount > 1 ? "s" : "" }}
				</span>
			</div>
		</div>
		...
		<div class="flex items-center gap-1 border-b border-white/10 px-3 py-2">
			...
			>
				<component :is="ar.icon" class="size-3" />
				{{ ar.label }}
				<span
		...
		<div class="flex-1 overflow-y-auto p-3 space-y-4">
			...
			<div>
				<div class="mb-1.5 flex items-center justify-between">
					<div class="flex items-center gap-1.5">
						<ImageIcon class="size-3.5 text-zinc-400" />
						<span class="text-[11px] font-medium text-zinc-300">Watermark</span>
					</div>
					<span
						v-if="configuredWatermarkRatios.length > 0"
						class="text-[10px] text-zinc-500"
					>
						{{ configuredWatermarkRatios.join(', ') }}
					</span>
				</div>
				...
				>
					<div
						v-if="activeWatermarkThumbnail"
						class="size-6 shrink-0 overflow-hidden rounded border border-white/10 bg-zinc-800"
					>
						<img :src="activeWatermarkThumbnail" class="size-full object-contain" />
					</div>
					<div v-else class="flex size-6 shrink-0 items-center justify-center rounded border border-white/10 bg-zinc-800">
						<Move class="size-3 text-zinc-500" />
					</div>
					<div class="flex-1 min-w-0">
						<span class="block truncate" :class="activeWatermarkName ? 'text-zinc-200' : 'text-zinc-500'">
							{{ activeWatermarkName ?? "Configure Watermark" }}
						</span>
						<span v-if="activeWatermark?.position" class="block text-[10px] text-zinc-500">
			...
			<div>
				<div class="mb-1.5 flex items-center gap-1.5">
					<Film class="size-3.5 text-zinc-400" />
					<span class="text-[11px] font-medium text-zinc-300">Intro Video</span>
				</div>
				...
				<div class="relative">
					...
					>
						<Film class="size-4 shrink-0 text-zinc-500" />
						<span class="flex-1 truncate" :class="activeIntroName ? 'text-zinc-200' : 'text-zinc-500'">
							{{ activeIntroName ?? "None" }}
						</span>
						<span v-if="activeIntro?.duration" class="text-[10px] text-zinc-500">
			...
			<div>
				<div class="mb-1.5 flex items-center gap-1.5">
					<Film class="size-3.5 text-zinc-400" />
					<span class="text-[11px] font-medium text-zinc-300">Outro Video</span>
				</div>
				...
				<div class="relative">
					...
					>
						<Film class="size-4 shrink-0 text-zinc-500" />
						<span class="flex-1 truncate" :class="activeOutroName ? 'text-zinc-200' : 'text-zinc-500'">
							{{ activeOutroName ?? "None" }}
						</span>
						<span v-if="activeOutro?.duration" class="text-[10px] text-zinc-500">
