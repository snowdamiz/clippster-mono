defmodule ClippsterServer.ClipValidation do
      ...
      if length(words) == 0 do
        ...
        case transcript do
          %{"segments" => segments} when is_list(segments) and length(segments) > 0 ->
            Logger.warning(
              "[ClipValidation] Found #{length(segments)} segments, attempting segment-level validation"
            )

            first_segment = hd(segments)

            Logger.info(
              "[ClipValidation] First segment keys: #{inspect(Map.keys(first_segment))}"
            )
            ...
            case validate_and_correct_clips_with_segments(clips, segments, verbose) do
              ...
              {:error, reason} ->
                Logger.warning(
                  "[ClipValidation] Segment-level validation failed: #{inspect(reason)}"
                )

                {:ok,
                 %{
                   validatedClips: clips,
                   qualityScore: 0.3,
                   issues: ["Limited validation: using AI timestamps only"],
                   corrections: []
                 }}
            end
          ...
          _ ->
            ...
            {:ok,
             %{
               validatedClips: clips,
               qualityScore: 0.0,
               issues: ["No timestamps available for validation"],
               corrections: []
             }}
        end
      else
        ...
        corrected_clips =
          ...
          |> Enum.map(fn {clip, clip_index} ->
            try do
              ...
              error ->
                Logger.error(
                  "[ClipValidation] Error validating clip #{clip_index}: #{inspect(error)}"
                )
        ...
        {total_issues, total_corrections, quality_scores} =
          corrected_clips
          |> Enum.reduce({[], [], []}, fn clip_result,
                                          {issues_acc, corrections_acc, scores_acc} ->
            try do
              ...
              {issues_acc ++ issues, corrections_acc ++ corrections,
               scores_acc ++ [quality_score]}
            rescue
              error ->
        ...
        Logger.info(
          "[ClipValidation] Validation completed. Quality score: #{Float.round(avg_quality_score, 2)}"
        )

        Logger.info(
          "[ClipValidation] Issues found: #{length(total_issues)}, Corrections applied: #{length(total_corrections)}"
        )
  ...
  def validate_and_correct_clip(clip, words, clip_index, verbose) do
    Logger.debug(
      "[ClipValidation] AI-enhanced validation of clip #{clip_index}: #{clip["title"] || "Untitled"}"
    )
    ...
    {validated_segments, segment_issues, segment_corrections} =
      ...
      |> Enum.reduce({[], [], []}, fn {segment, issues, corrections},
                                      {segments_acc, issues_acc, corrections_acc} ->
    ...
    |> Map.put(
      "qualityScore",
      calculate_ai_enhanced_quality_score(validated_clip, segment_issues, segment_corrections)
    )
  end
  ...
  """
  @spec validate_and_correct_segment(map(), list(word()), integer(), integer(), boolean()) ::
          {map(), list(String.t()), list(String.t())}
  def validate_and_correct_segment(segment, words, clip_index, segment_index, verbose) do
    transcript_text = segment["transcript"]

    if verbose do
      Logger.debug(
        "[ClipValidation] Segment #{clip_index}.#{segment_index}: \"#{String.slice(transcript_text, 0, 50)}...\""
      )
    end
    ...
    try do
      case match_result do
        nil ->
          if verbose do
            Logger.warning(
              "[ClipValidation] No match found for segment #{clip_index}.#{segment_index}"
            )
          end
        ...
        match ->
          ...
          if match != nil and confidence < @fuzzy_match_threshold do
            if verbose do
              Logger.warning(
                "[ClipValidation] Low confidence match (#{Float.round(confidence * 100, 1)}%) for segment #{clip_index}.#{segment_index}"
              )
            end
            ...
            if match != nil and verbose do
              Logger.info(
                "[ClipValidation] Match found (#{Float.round(confidence * 100, 1)}% confidence)"
              )

              Logger.debug(
                "[ClipValidation] Original: #{segment["start_time"]} - #{segment["end_time"]}"
              )

              Logger.debug(
                "[ClipValidation] Corrected: #{Map.get(match, :startTime, "?")} - #{Map.get(match, :endTime, "?")}"
              )
            end
            ...
            corrected_segment =
              if match != nil do
                try do
                  ...
                  Logger.debug(
                    "[ClipValidation] Building corrected_segment with: start_time=#{start_time}, end_time=#{end_time}, word_indices=#{inspect(word_indices)}"
                  )
                  ...
                  error ->
                    Logger.error(
                      "[ClipValidation] Error creating corrected_segment: #{inspect(error)}"
                    )
                    ...
                    if match != nil do
                      ...
                      Logger.error(
                        "[ClipValidation] match.wordIndices: #{inspect(Map.get(match, :wordIndices, "NOT_FOUND"))}"
                      )
                    end
            ...
            corrected_segment =
              try do
                ...
                error ->
                  Logger.error(
                    "[ClipValidation] Error adding validation_confidence: #{inspect(error)}"
                  )

                  Logger.error("[ClipValidation] confidence: #{inspect(confidence)}")

                  Logger.error(
                    "[ClipValidation] corrected_segment before error: #{inspect(corrected_segment)}"
                  )

                  # Return original segment if error occurs
                  corrected_segment
              end
            ...
            try do
              match_start_time =
                if match != nil,
                  do: Map.get(match, :startTime, segment["start_time"]),
                  else: segment["start_time"]

              match_end_time =
                if match != nil,
                  do: Map.get(match, :endTime, segment["end_time"]),
      ...
      error ->
        Logger.error(
          "[ClipValidation] Unexpected error in validate_and_correct_segment: #{inspect(error)}"
        )
  ...
  """
  @spec find_transcript_match(String.t(), list(word()), float(), boolean()) ::
          transcript_match() | nil
  ...
  """
  @spec find_exact_match(list(String.t()), list(word()), float(), boolean()) ::
          transcript_match() | nil
  def find_exact_match(target_words, transcript_words, time_hint \\ nil, verbose \\ false) do
    ...
    search_end =
      Enum.min([length(transcript_words) - total_target_words, start_index + @max_search_radius])
  ...
  """
  @spec find_fuzzy_match(list(String.t()), list(word()), float(), boolean()) ::
          transcript_match() | nil
  ...
  defp normalize_word(word) do
    word
    |> String.downcase()
    # Handle contractions like "don't" -> "dont"
    |> String.replace("'", "")
  end
  ...
  defp count_word_matches(target_words, candidate_words, allow_levenshtein) do
    ...
    |> Enum.count(fn {target_word, index} ->
      case Enum.at(candidate_words, index) do
        ...
        candidate_word when candidate_word == target_word ->
          true
  ...
  defp calculate_distance_recursive(
         <<s_char::utf8, s_rest::binary>>,
         <<t_char::utf8, t_rest::binary>>,
         current_distance,
         max_distance
       ) do
    if current_distance > max_distance do
      ...
      substitute_cost =
        calculate_distance_recursive(s_rest, t_rest, current_distance + cost, max_distance)
  ...
  defp validate_and_correct_clips_with_segments(clips, segments, verbose) do
    ...
    try do
      ...
      {validated_clips, all_issues, all_corrections} =
        ...
        |> Enum.reduce({[], [], []}, fn {validated_clip, issues, corrections},
                                        {clips_acc, issues_acc, corrections_acc} ->
      ...
      # Start lower since we can't do word-level precision
      base_score = 0.6
      ...
      Logger.info(
        "[ClipValidation] Segment-level validation completed. Quality score: #{Float.round(quality_score, 2)}"
      )
  ...
  defp validate_clip_with_segments(clip, segments, clip_index, verbose) do
    Logger.debug(
      "[ClipValidation] Validating clip #{clip_index} against #{length(segments)} segments"
    )
    ...
    {validated_segments, segment_issues, segment_corrections} =
      ...
      |> Enum.reduce({[], [], []}, fn {validated_segment, issues, corrections},
                                      {segments_acc, issues_acc, corrections_acc} ->
  ...
  defp validate_segment_with_segments(
         clip_segment,
         transcript_segments,
         clip_index,
         segment_index,
    ...
    if verbose do
      Logger.debug(
        "[ClipValidation] Validating segment #{clip_index}.#{segment_index}: #{clip_segment["start_time"]} - #{clip_segment["end_time"]}"
      )
    end
    ...
    validated_segment =
      if length(start_corrections) > 0 or length(end_corrections) > 0 do
        %{
          clip_segment
  ...
  defp validate_ai_enhanced_segment(segment, _words, clip_index, segment_index, verbose) do
    if verbose do
      Logger.debug(
        "[ClipValidation] AI-enhanced validation of segment #{clip_index}.#{segment_index}"
      )
    end
  ...
  defp validate_ai_splicing_decisions(segment) do
    ...
    cond do
      ...
      has_dead_space and
          not Enum.any?(internal_gaps, fn gap ->
  ...
  defp calculate_ai_enhanced_quality_score(clip, issues, corrections) do
    ...
    # Up to 0.2 bonus for high density
    density_bonus = avg_density * 0.2
    ...
    final_score =
      base_score + density_bonus + rate_bonus + splicing_bonus - issue_penalty -
        correction_penalty
  ...
  def filter_by_minimum_duration(clips, min_duration_seconds)
      when is_list(clips) and is_number(min_duration_seconds) do
    Logger.info(
      "[ClipValidation] Filtering clips by minimum duration: #{min_duration_seconds} seconds"
    )

    {filtered_clips, removed_count} =
      clips
      |> Enum.reduce({[], 0}, fn clip, {keep_acc, remove_count} ->
        ...
        if clip_duration >= min_duration_seconds do
          ...
          Logger.debug(
            "[ClipValidation] Removing clip '#{clip["title"] || "Untitled"}' - duration #{Float.round(clip_duration, 2)}s < #{min_duration_seconds}s minimum"
          )
    ...
    if removed_count > 0 do
      Logger.info(
        "[ClipValidation] Filtered out #{removed_count} clip(s) below minimum duration of #{min_duration_seconds}s"
      )
    end
  ...
  defp get_clip_duration(clip) when is_map(clip) do
    cond do
      ...
      Map.has_key?(clip, "segments") and is_list(clip["segments"]) and
          length(clip["segments"]) > 0 ->
      ...
      true ->
        Logger.warning(
          "[ClipValidation] Could not determine duration for clip: #{inspect(Map.keys(clip))}"
        )
