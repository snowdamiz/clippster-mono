defmodule ClippsterServer.AI.SystemPrompt do
  ...
  **Key Requirements:**
  ...
  - All timestamps in seconds (decimal precision) and within {START_TIME}–{END_TIME}.
  ...
  **Filename Guidelines:**
  ...
  @doc """
  Returns the system prompt with placeholders for dynamic content.

  ## Placeholders
  - `{START_TIME}` - Start time of the transcript chunk
  - `{END_TIME}` - End time of the transcript chunk
  - `{TRANSCRIPT_CONTENT}` - The actual transcript content
  ...
  @doc """
  Returns the system prompt with placeholders replaced by actual values.

  ## Parameters
  - `start_time` - Start time in seconds (float or integer)
  - `end_time` - End time in seconds (float or integer)
  - `transcript_content` - The transcript text content

  ## Example
      iex> SystemPrompt.build(0.0, 120.5, "Speaker: Hello world")
      "**BOUNDARY SELECTION ALGORITHM...Time range: 0.0 to 120.5..."
  """
  def build(start_time, end_time, transcript_content) do
    @system_prompt
    |> String.replace("{START_TIME}", to_string(start_time))
    |> String.replace("{END_TIME}", to_string(end_time))
    |> String.replace("{TRANSCRIPT_CONTENT}", transcript_content)
  end
end
