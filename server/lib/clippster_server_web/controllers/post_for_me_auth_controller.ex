defmodule ClippsterServerWeb.PostForMeAuthController do
  ...
  defp html_page(title, message, color) do
    icon_svg =
      if color == "#22c55e" do
        ~s(<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>)
      else
        ~s(<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>)
      end
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .gradient-overlay { position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255,255,255,0.02) 0%, transparent 50%, #{color}0d 100%); pointer-events: none; }
        .content { position: relative; padding: 2.5rem 2rem; text-align: center; }
        .icon-container { width: 64px; height: 64px; margin: 0 auto 1.5rem; background: #{color}1a; border: 1px solid #{color}33; border-radius: 1rem; display: flex; align-items: center; justify-content: center; animation: scaleIn 0.5s ease-out; }
        .icon-container svg { width: 32px; height: 32px; color: #{color}; }
        h1 { font-size: 1.25rem; font-weight: 700; margin-bottom: 0.5rem; color: #{color}; }
