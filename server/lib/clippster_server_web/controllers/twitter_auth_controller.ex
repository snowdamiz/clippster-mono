defmodule ClippsterServerWeb.TwitterAuthController do
  ...
  @oauth_context_salt "org_twitter_oauth_context"
      ...
      {:ok, user} ->
        if Organizations.is_admin?(org_id, user.id) do
          ...
          if is_nil(client_id) do
            ...
            server_callback_url =
              ClippsterServerWeb.Endpoint.url() <> "/api/auth/twitter/callback"
          ...
          redirect_with_error(
            conn,
            callback_target,
            "Only organization admins can connect X accounts"
          )
        end
  ...
  def oauth_callback(conn, %{"code" => code, "state" => state_encoded}) do
    case decode_state(state_encoded) do
      {:ok, %{"code_verifier" => code_verifier, "ctx" => context_token}} ->
