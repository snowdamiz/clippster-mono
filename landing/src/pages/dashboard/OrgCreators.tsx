...
import { deleteOrganizationCreatorProfile, uploadCreatorProfileImage, assignProfile, unassignProfile } from '@/services/organizationApi'
...
export function OrgCreators() {
  const { creatorProfiles, profilesLoading, loadCreatorProfiles, organizationId, isAdmin, members, loadOrganization } = useOrganization()
  ...
  // Computed stats
  const totalPlatformLinks = useMemo(() => creatorProfiles.reduce((sum, p) => sum + (p.platform_links?.length || 0), 0), [creatorProfiles])
  const totalAssignedMembers = useMemo(() => creatorProfiles.reduce((sum, p) => sum + (p.assigned_count || p.assignments?.length || 0), 0), [creatorProfiles])

  function getMenuPosition(profileId: number): React.CSSProperties {
    const btn = menuButtonRefs.current.get(profileId)
    if (!btn) return { top: 0, left: 0 }
    const rect = btn.getBoundingClientRect()
    const menuWidth = 200; const menuMaxHeight = 160; const padding = 8
    let top = rect.bottom + padding; let left = rect.right - menuWidth
  ...
  async function executeDelete() {
    ...
    try {
      ...
      if (result.success) {
        ...
        loadCreatorProfiles()
        setShowDeleteDialog(false); setDeleteDialogProfile(null)
      } else {
  ...
  async function saveAssignments() {
    ...
    try {
      ...
      loadCreatorProfiles()
      setShowAssignmentDialog(false); setAssignmentProfile(null)
    } catch (err: any) {
    ...
    <PageLayout icon={UserCircle} title="Creator Profiles" description="Manage creator profiles and assign them to team members" actions={
      ...
      <div className="oc">
        ...
        <div className="oc-heading">
          ...
          <p className="oc-heading__subtitle">Create and manage creator profiles with platform links, intros, outros, and watermarks</p>
        </div>
        ...
        <div className="oc-stats">
          <div className="oc-stats__card">
            ...
            <div className="oc-stats__inner">
              <div className="oc-stats__icon oc-stats__icon--green"><UserCircle className="w-5 h-5" /></div>
          ...
          <div className="oc-stats__card">
            ...
            <div className="oc-stats__inner">
              <div className="oc-stats__icon oc-stats__icon--purple"><Link2 className="w-5 h-5" /></div>
              <div className="oc-stats__text">
        ...
        <section className="oc-section">
          <div className="oc-section__header">
            <div className="oc-section__header-icon"><Layers className="w-5 h-5" /></div>
                ...
                <div key={profile.id} className="oc-card">
                  ...
                  <div className="oc-card__header">
                    <div className="oc-card__avatar">
                      {profile.profile_image_url ? (
                        ...
                        <div className="oc-card__avatar-fallback"><UserCircle className="w-6 h-6 text-zinc-500 opacity-60" /></div>
                      )}
                    </div>
                    ...
                    {isAdmin && (
                      <div className="oc-card__actions">
                        <button
                          ref={el => { if (el) menuButtonRefs.current.set(profile.id, el); else menuButtonRefs.current.delete(profile.id) }}
                        ...
                        {openMenuId === profile.id && createPortal(
                          <div className="oc-dropdown" style={getMenuPosition(profile.id)} onClick={e => e.stopPropagation()}>
                            ...
                            <button className="oc-dropdown__item oc-dropdown__item--danger" onClick={() => { setDeleteDialogProfile(profile); setShowDeleteDialog(true); setOpenMenuId(null) }}>
                  ...
                  <div className="oc-card__stats-row">
                      ...
                      {profile.platform_links && profile.platform_links.length > 0 ? (
                        <>
                          {profile.platform_links.slice(0, 4).map(link => (
                            <div key={link.id} className="oc-card__platform-icon-wrapper" title={link.display_name || link.platform}>
                              <img src={getPlatformIcon(link.platform)} alt={link.platform} className="oc-card__platform-icon" style={{ filter: getPlatformFilter(link.platform) }} />
                            </div>
                          ))}
                          {profile.platform_links.length > 4 && <span className="oc-card__more-badge">+{profile.platform_links.length - 4}</span>}
                        </>
                      ) : (
                        <span className="oc-card__empty-indicator"><Link2 className="w-4 h-4 text-zinc-500" /></span>
                      )}
                    </div>
                    ...
                    <div className="oc-card__branding">
                      <div className={`oc-card__branding-icon ${profile.intro_id ? 'oc-card__branding-icon--active' : ''}`} title={profile.intro_id ? 'Intro configured' : 'No intro'}><Play className="w-3.5 h-3.5" /></div>
                      <div className={`oc-card__branding-icon ${profile.outro_id ? 'oc-card__branding-icon--active' : ''}`} title={profile.outro_id ? 'Outro configured' : 'No outro'}><SkipForward className="w-3.5 h-3.5" /></div>
                      <div className={`oc-card__branding-icon ${profile.watermark_id ? 'oc-card__branding-icon--active' : ''}`} title={profile.watermark_id ? 'Watermark configured' : 'No watermark'}><ImageIcon className="w-3.5 h-3.5" /></div>
                    </div>
                        ...
                        <>
                          <div className="oc-card__avatar-stack">
                            {profile.assignments.slice(0, 3).map((assignment, idx) => (
                              <div key={assignment.id} className="oc-card__stack-avatar" style={{ zIndex: 3 - idx }} title={assignment.user?.name || assignment.user?.email || 'Member'}>
                                {assignment.user?.avatar_url ? (
                                  <img src={assignment.user.avatar_url} alt={assignment.user?.name || 'Member'} className="oc-card__stack-avatar-img" />
                                ) : (
                          ...
                          {(profile.assigned_count || profile.assignments.length) > 3 && <span className="oc-card__more-badge">+{(profile.assigned_count || profile.assignments.length) - 3}</span>}
                        </>
                      ) : (
                        <span className="oc-card__empty-indicator"><Users className="w-4 h-4 text-zinc-500" /></span>
                      )}
                    </div>
                  </div>
      ...
      {showDeleteDialog && createPortal(
        <div className="om-dialog__overlay" onClick={e => { if (e.target === e.currentTarget && !deleteProcessing) { setShowDeleteDialog(false); setDeleteDialogProfile(null) } }}>
          <div className="om-dialog om-dialog--red">
            ...
            <div className="om-dialog__header">
              <button className="om-dialog__close" onClick={() => { setShowDeleteDialog(false); setDeleteDialogProfile(null) }} disabled={deleteProcessing}><X size={18} /></button>
            ...
            <div className="om-dialog__content">
              <div className="oc-dialog__profile-card">
                <div className="oc-dialog__profile-avatar">
                  {deleteDialogProfile?.profile_image_url ? (
                    <img src={deleteDialogProfile.profile_image_url} alt={deleteDialogProfile.name} className="oc-dialog__profile-avatar-img" />
                  ) : (
                ...
                <div className="oc-dialog__profile-info">
                  ...
                  <div className="oc-dialog__profile-desc">{deleteDialogProfile?.description || 'No description'}</div>
                </div>
              ...
              <div className="om-dialog__warning-box">
                <p className="om-dialog__warning-text">
                  ...
                  {(deleteDialogProfile?.assigned_count || 0) > 0 && ` This will unassign ${deleteDialogProfile?.assigned_count} ${deleteDialogProfile?.assigned_count === 1 ? 'member' : 'members'}.`}
                </p>
              </div>
            </div>
            <div className="om-dialog__footer">
              <button className="om-dialog__btn om-dialog__btn--secondary" onClick={() => { setShowDeleteDialog(false); setDeleteDialogProfile(null) }} disabled={deleteProcessing}>Cancel</button>
              <button className="om-dialog__btn om-dialog__btn--danger" onClick={executeDelete} disabled={deleteProcessing}>
      ...
      {/* Assignment Dialog */}
      {showAssignmentDialog && createPortal(
        <div className="om-dialog__overlay" onClick={e => { if (e.target === e.currentTarget && !assignmentSaving) { setShowAssignmentDialog(false); setAssignmentProfile(null) } }}>
          <div className="om-dialog om-dialog--cyan">
            <div className="om-dialog__accent om-dialog__accent--cyan" />
            <div className="om-dialog__header">
              <button className="om-dialog__close" onClick={() => { setShowAssignmentDialog(false); setAssignmentProfile(null) }} disabled={assignmentSaving}><X size={18} /></button>
              <div className="om-dialog__icon om-dialog__icon--cyan"><Users size={24} /></div>
              ...
              {assignmentLoading ? (
                ...
                <div className="oc-dialog__member-list">
                  {members.length > 0 ? members.map(member => (
                    ...
                    >
                      <div className={`oc-dialog__checkbox ${selectedUserIds.includes(member.user_id) ? 'oc-dialog__checkbox--checked' : ''}`}>
                        {selectedUserIds.includes(member.user_id) && <Check className="w-3 h-3 text-white" />}
                      </div>
                      <div className="oc-dialog__member-avatar">
                        {member.user?.avatar_url ? (
                          <img src={member.user.avatar_url} alt={member.user.name || member.user.email} className="oc-dialog__member-avatar-img" referrerPolicy="no-referrer" />
                        ) : (
                      ...
                      <span className={`oc-dialog__role-badge ${member.role === 'owner' ? 'oc-dialog__role-badge--owner' : member.role === 'admin' ? 'oc-dialog__role-badge--admin' : ''}`}>{member.role}</span>
                    </div>
                  )) : (
                    <div className="oc-dialog__empty-members">
                      <div className="oc-dialog__empty-members-icon"><Users className="w-6 h-6" /></div>
            ...
            {!assignmentLoading && members.length > 0 && (
              <div className="oc-dialog__summary">
                <div className="oc-dialog__summary-row">
                  <span className="oc-dialog__summary-text">{selectedUserIds.length} of {members.length} members selected</span>
                  <div className="oc-dialog__summary-actions">
                    <button type="button" onClick={() => setSelectedUserIds(members.map(m => m.user_id))} className="oc-dialog__summary-link">Select All</button>
                    <span className="oc-dialog__summary-divider">|</span>
                    <button type="button" onClick={() => setSelectedUserIds([])} className="oc-dialog__summary-link oc-dialog__summary-link--muted">Clear</button>
                  </div>
                </div>
              </div>
            )}
            <div className="om-dialog__footer" style={{ marginTop: 0 }}>
              <button className="om-dialog__btn om-dialog__btn--secondary" onClick={() => { setShowAssignmentDialog(false); setAssignmentProfile(null) }} disabled={assignmentSaving}>Cancel</button>
              <button className="om-dialog__btn om-dialog__btn--primary" onClick={saveAssignments} disabled={assignmentSaving}>
