...
import {
  ...
  type HiringPost, type HiringApplication, type HiringPostFormData,
...
export function OrgHiring() {
  ...
  useEffect(() => { if (hiringPost) loadApplications() }, [hiringPost?.id])
  ...
  const postStatusColor = !hiringPost ? 'text-zinc-500' : hiringPost.status === 'active' ? 'text-green-500' : hiringPost.status === 'paused' ? 'text-yellow-500' : 'text-zinc-500'
    ...
    return (
      <>
        <div className="mb-2">
          ...
          <p className="text-sm text-zinc-500 mt-1">Create a hiring post, review applications, and recruit clippers to your organization</p>
        </div>
          ...
          <div className="relative flex bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden hover:border-white/10 hover:shadow-lg transition-all">
            ...
            <div className="flex-1 flex items-center gap-4 px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[0.6875rem] text-zinc-500 mt-0.5 truncate">{hiringPost ? hiringPost.title : 'No active post'}</p>
              </div>
              <span className={`text-[1.75rem] font-bold tracking-tight tabular-nums shrink-0 ${postStatusColor}`}>{postStatusLabel}</span>
            </div>
          </div>
          {/* Applicants */}
          <div className="relative flex bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden hover:border-white/10 hover:shadow-lg transition-all">
            ...
            <div className="flex-1 flex items-center gap-4 px-5 py-4">
              ...
              <span className="text-[1.75rem] font-bold tracking-tight tabular-nums shrink-0 text-violet-400">{applications.length}</span>
            </div>
          </div>
          {/* Slots Filled */}
          <div className="relative flex bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden hover:border-white/10 hover:shadow-lg transition-all">
            ...
            <div className="flex-1 flex items-center gap-4 px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[0.6875rem] text-zinc-500 mt-0.5">{hiringPost?.clipper_slots ? `${hiringPost.clipper_slots - (hiringPost.clipper_slots_filled || 0)} remaining` : 'No limit set'}</p>
              </div>
    ...
    return (
      <PageLayout icon={Briefcase} title="Hiring">
        ...
        <div className="flex flex-col items-center justify-center text-center py-16 px-8">
          ...
          <p className="text-sm text-zinc-500 mb-6 max-w-[320px]">Create a hiring post to let clippers know you're looking for talent</p>
          <button
      ...
      <PageLayout icon={Briefcase} title="Hiring">
        ...
        <div className="max-w-[800px] mx-auto p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            {hiringPost && (
              <button onClick={() => setShowForm(false)} className="px-3 py-1.5 rounded-md border border-zinc-700 bg-transparent text-zinc-400 text-[0.8125rem] cursor-pointer hover:text-white hover:border-zinc-600 transition-all">
                Cancel
              </button>
            )}
          </div>

          <form onSubmit={e => { e.preventDefault(); savePost() }} className="flex flex-col gap-6">
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {SPECIALTY_TAGS.map(tag => (
                    <button key={tag.value} type="button"
                      onClick={() => updateForm({ content_types: toggleArrayItem(form.content_types || [], tag.value) })}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input value={form.payment_details || ''} onChange={e => updateForm({ payment_details: e.target.value })}
                  className="px-3 py-2 rounded-lg border border-zinc-700 bg-zinc-900 text-white text-sm outline-none focus:border-cyan-400 transition-colors"
              ...
              {/* Experience Level */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Minimum Experience</label>
                <select value={form.experience_level || ''} onChange={e => updateForm({ experience_level: e.target.value })}
                  className="px-3 py-2 rounded-lg border border-zinc-700 bg-zinc-900 text-white text-sm outline-none focus:border-cyan-400 cursor-pointer appearance-none">
              ...
              {/* Streamer Count */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Number of Streamers</label>
                <input type="number" min={0} value={form.streamer_count ?? ''} onChange={e => updateForm({ streamer_count: e.target.value ? Number(e.target.value) : undefined })}
                  className="px-3 py-2 rounded-lg border border-zinc-700 bg-zinc-900 text-white text-sm outline-none focus:border-cyan-400 transition-colors"
                  placeholder="e.g. 5" />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input type="number" min={1} value={form.clipper_slots ?? ''} onChange={e => updateForm({ clipper_slots: e.target.value ? Number(e.target.value) : undefined })}
                  className="px-3 py-2 rounded-lg border border-zinc-700 bg-zinc-900 text-white text-sm outline-none focus:border-cyan-400 transition-colors"
                  placeholder="e.g. 10" />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => updateForm({ is_public: !form.is_public })}
                  ...
                  <span className="text-[0.8125rem] text-zinc-400">{form.is_public ? 'Visible to all clippers' : 'Hidden from directory'}</span>
                </div>
              </div>
            </div>
    ...
    <PageLayout icon={Briefcase} title="Hiring">
        ...
        <>
            ...
            <div className="flex flex-col gap-3.5 p-4 flex-1">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`px-2 py-0.5 rounded-[5px] text-[0.625rem] font-bold uppercase tracking-wider backdrop-blur-sm shadow-sm ${
                    hiringPost.status === 'active' ? 'bg-emerald-500/95 text-white' :
                    hiringPost.status === 'paused' ? 'bg-amber-500/95 text-white' :
                    'bg-zinc-500/90 text-white'
                  }`}>{hiringPost.status}</span>
                  <span className={`px-2 py-0.5 rounded-[5px] text-[0.625rem] font-bold uppercase tracking-wider ${
                    hiringPost.is_public ? 'bg-blue-500/15 text-blue-400' : 'bg-zinc-500/15 text-zinc-400'
                  }`}>{hiringPost.is_public ? 'Public' : 'Hidden'}</span>
                </div>
                {hiringPost.description && <p className="text-[0.8125rem] text-zinc-400 m-0 leading-relaxed line-clamp-2">{hiringPost.description}</p>}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(hiringPost.content_types || []).slice(0, 4).map(t => (
                    <span key={t} className="px-2 py-0.5 rounded-full text-[0.6875rem] bg-white/[0.06] text-zinc-400">{getSpecialtyTagLabel(t)}</span>
                  ))}
                  {(hiringPost.platforms || []).slice(0, 3).map(p => (
                    <span key={p} className="px-2 py-0.5 rounded-full text-[0.6875rem] bg-cyan-400/10 text-cyan-400">{getPlatformLabel(p)}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 bg-zinc-800/50 rounded-lg overflow-hidden">
                <div className="flex flex-col items-center justify-center gap-0.5 py-3 px-2 text-center border-r border-zinc-700/50">
                  ...
                  <span className="text-[0.5625rem] text-zinc-400 uppercase tracking-wider font-medium">slots filled</span>
                </div>
                <div className="flex flex-col items-center justify-center gap-0.5 py-3 px-2 text-center border-r border-zinc-700/50">
                  <span className="text-[0.9375rem] font-bold text-white tabular-nums leading-tight">{applications.length}</span>
                  <span className="text-[0.5625rem] text-zinc-400 uppercase tracking-wider font-medium">applicants</span>
                </div>
                <div className="flex flex-col items-center justify-center gap-0.5 py-3 px-2 text-center">
                  <span className="text-[0.9375rem] font-bold text-white tabular-nums leading-tight">{hiringPost.streamer_count || 0}</span>
                  ...
                  <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden border border-zinc-700/50">
                    <div
                      className={`h-full rounded-full transition-all duration-400 ${
                        ((hiringPost.clipper_slots_filled || 0) / hiringPost.clipper_slots) >= 0.8 ? 'bg-gradient-to-r from-red-500 to-red-600' :
                        ((hiringPost.clipper_slots_filled || 0) / hiringPost.clipper_slots) >= 0.5 ? 'bg-gradient-to-r from-amber-500 to-amber-600' :
              ...
              <div className="flex items-center justify-center gap-2 pt-3 mt-auto border-t border-zinc-700/50">
                <button onClick={startEdit} title="Edit" className="flex items-center justify-center flex-1 h-[34px] max-w-[100px] bg-transparent border border-zinc-700 rounded-lg text-zinc-400 cursor-pointer transition-all hover:bg-zinc-800 hover:border-cyan-400 hover:text-cyan-400">
                  <Pencil className="w-4 h-4" />
                </button>
                <button onClick={togglePause} title={hiringPost.status === 'active' ? 'Pause' : 'Activate'} className="flex items-center justify-center flex-1 h-[34px] max-w-[100px] bg-transparent border border-zinc-700 rounded-lg text-zinc-400 cursor-pointer transition-all hover:bg-zinc-800 hover:border-cyan-400 hover:text-cyan-400">
                  {hiringPost.status === 'active' ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                </button>
                <button onClick={() => setShowDeleteDialog(true)} title="Delete" className="flex items-center justify-center flex-1 h-[34px] max-w-[100px] bg-transparent border border-zinc-700 rounded-lg text-zinc-400 cursor-pointer transition-all hover:bg-zinc-800 hover:border-red-500 hover:text-red-500">
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-white m-0 flex items-center gap-2">
                ...
                {applications.length > 0 && (
                  <span className="inline-flex items-center justify-center min-w-5 h-5 px-1.5 rounded-full bg-cyan-400/15 text-cyan-400 text-[0.6875rem] font-bold">{applications.length}</span>
                )}
              </h3>
              <button onClick={loadApplications} disabled={loadingApps}
            ...
            {loadingApps && !applications.length ? (
              ...
              <p className="text-center py-8 text-zinc-500 text-sm">No applications yet. Clippers will appear here when they apply.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {applications.map(app => (
                  <div key={app.id} className="flex flex-col bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden transition-all hover:border-white/15 hover:shadow-lg hover:shadow-black/20">
                    {/* Status Bar */}
                    <div className="flex items-center justify-between px-4 py-2 bg-gradient-to-r from-black/20 to-black/10 border-b border-zinc-800">
                      <span className={`flex items-center gap-1.5 px-2.5 py-0.5 text-[0.6875rem] font-semibold uppercase tracking-wider rounded-full ${
                        app.status === 'pending' ? 'bg-gradient-to-r from-yellow-500/20 to-yellow-500/10 text-yellow-500' :
                        app.status === 'accepted' ? 'bg-gradient-to-r from-emerald-500/20 to-emerald-500/10 text-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.15)]' :
                    ...
                    {app.clipper_profile?.slug ? (
                      <a href={`/clippers/${app.clipper_profile.slug}`} className="flex flex-col gap-3.5 p-4 no-underline text-inherit transition-colors hover:bg-white/[0.02]">
                        <div className="flex gap-3.5">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 text-[0.9375rem] font-semibold text-white mb-0.5">
                              ...
                              {app.clipper_profile?.is_verified && <CheckCircle className="w-3.5 h-3.5 text-blue-500 shrink-0" />}
                            </div>
                            {app.clipper_profile?.experience_level && (
                              <div className="text-[0.6875rem] font-medium text-cyan-400 uppercase tracking-wider mb-1.5">{getExperienceLevelLabel(app.clipper_profile.experience_level)}</div>
                            )}
                            {app.message && <p className="text-[0.8125rem] text-zinc-400 m-0 leading-relaxed line-clamp-2">{app.message}</p>}
                          </div>
                        </div>
                        ...
                        {app.clipper_profile && (
                          <div className="flex items-center gap-4 px-3 py-2.5 bg-black/15 rounded-lg">
                            <span className="flex items-center gap-1"><Video className="w-[13px] h-[13px] text-zinc-400 opacity-70" /><span className="text-[0.8125rem] font-semibold text-white">{app.clipper_profile.total_clips_delivered || 0}</span><span className="text-[0.6875rem] text-zinc-400">clips</span></span>
                            <span className="flex items-center gap-1"><Star className="w-[13px] h-[13px] text-yellow-500" /><span className="text-[0.8125rem] font-semibold text-white">{app.clipper_profile.total_endorsements || 0}</span><span className="text-[0.6875rem] text-zinc-400">reviews</span></span>
                            <span className="flex items-center gap-1"><Briefcase className="w-[13px] h-[13px] text-cyan-400 opacity-80" /><span className="text-[0.8125rem] font-semibold text-white">{app.clipper_profile.total_campaigns_completed || 0}</span><span className="text-[0.6875rem] text-zinc-400">campaigns</span></span>
                          </div>
                        )}
                        ...
                        {app.clipper_profile?.specialty_tags?.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {app.clipper_profile.specialty_tags.slice(0, 3).map(tag => (
                              <span key={tag} className="px-2 py-0.5 rounded-full text-[0.6875rem] bg-white/[0.06] text-zinc-400 border border-zinc-700/50">{getSpecialtyTagLabel(tag)}</span>
                            ))}
                            {app.clipper_profile.specialty_tags.length > 3 && (
                              <span className="text-[0.6875rem] text-zinc-400 font-semibold ml-0.5">+{app.clipper_profile.specialty_tags.length - 3}</span>
                            )}
                          </div>
                        )}
                      </a>
                      ...
                      <div className="flex flex-col gap-3.5 p-4">
                        <div className="flex gap-3.5">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-[0.9375rem] font-semibold text-white mb-0.5">{app.user?.name || app.user?.email || 'Unknown'}</div>
                            {app.message && <p className="text-[0.8125rem] text-zinc-400 m-0 leading-relaxed line-clamp-2">{app.message}</p>}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-2 px-4 py-3 border-t border-zinc-800">
                      {app.clipper_profile?.user_id && (
                        <a href={`/dashboard/org/${organizationId}/messages?to=${app.clipper_profile.user_id}`}
                      ...
                      {app.status === 'pending' && (
                        <>
                          <button onClick={() => handleAccept(app)} disabled={accepting === app.id}
                            ...
                            {accepting === app.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle className="w-3.5 h-3.5" />} Hire
                          </button>
                          <button onClick={() => handleReject(app)} disabled={rejecting === app.id}
                            ...
                            {rejecting === app.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <X className="w-3.5 h-3.5" />} Reject
                          </button>
                        </>
                      )}
                    </div>
      ...
      {showDeleteDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowDeleteDialog(false)}>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 max-w-[420px] w-full mx-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-white m-0 mb-2">Delete Hiring Post</h3>
            <p className="text-sm text-zinc-400 m-0 mb-6">This will permanently delete your hiring post and all applications. This action cannot be undone.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowDeleteDialog(false)} className="px-4 py-2 rounded-md border border-zinc-700 bg-transparent text-zinc-400 text-sm font-semibold cursor-pointer hover:text-white transition-colors">Cancel</button>
              <button onClick={deletePost} disabled={deleting} className="flex items-center gap-2 px-4 py-2 rounded-md bg-red-500 text-white text-sm font-semibold border-none cursor-pointer hover:bg-red-600 transition-colors disabled:opacity-50">
      ...
      {showAcceptDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowAcceptDialog(false)}>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 max-w-[420px] w-full mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowAcceptDialog(false)} className="px-4 py-2 rounded-md border border-zinc-700 bg-transparent text-zinc-400 text-sm font-semibold cursor-pointer hover:text-white transition-colors">Cancel</button>
              <button onClick={confirmAccept} disabled={accepting !== null} className="flex items-center gap-2 px-4 py-2 rounded-md bg-cyan-400 text-[#0a0a0b] text-sm font-semibold border-none cursor-pointer hover:opacity-90 transition-opacity disabled:opacity-50">
