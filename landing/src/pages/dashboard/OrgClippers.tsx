...
function getPlatformIconClass(platform: string) {
  return (
    {
      tiktok: 'cl-platform-icon--tiktok',
      instagram: 'cl-platform-icon--instagram',
      youtube: 'cl-platform-icon--youtube',
      x: 'cl-platform-icon--x',
      facebook: 'cl-platform-icon--facebook',
      snapchat: 'cl-platform-icon--snapchat'
    }[platform] || ''
  )
}
function getPlatformLetter(platform: string) {
  return (
    { tiktok: 'T', instagram: 'I', youtube: 'Y', x: 'X', facebook: 'F', snapchat: 'S' }[platform] ||
...
export function OrgClippers() {
  ...
  const loadClippers = useCallback(
    async (currentFilters?: DirectoryFilters) => {
      setLoading(true)
      try {
      ...
      try {
        ...
        if (result.success) {
          setLeaderboardEntries(
            (result.entries || []).map((e: any, i: number) => ({ ...e, total_views: e.total_views || 0, rank: i + 1 }))
          )
        }
      } finally {
  ...
  const hasActiveFilters = useMemo(() => {
    return !!(
      ...
      (filters.experience_level && filters.experience_level !== 'any') ||
      filters.specialty_tags?.length ||
      filters.content_style_tags?.length ||
      filters.preferred_platforms?.length ||
  ...
  function getSpecialtiesFilterLabel() {
    ...
    if (filters.specialty_tags.length === 1)
      return SPECIALTY_TAGS.find((t) => t.value === filters.specialty_tags![0])?.label || 'Specialties'
  ...
  function getContentStyleFilterLabel() {
    ...
    if (filters.content_style_tags.length === 1)
      return CONTENT_STYLE_TAGS.find((t) => t.value === filters.content_style_tags![0])?.label || 'Content Style'
  ...
  function getPlatformsFilterLabel() {
    ...
    if (filters.preferred_platforms.length === 1)
      return PREFERRED_PLATFORMS.find((p) => p.value === filters.preferred_platforms![0])?.label || 'Platforms'
  ...
  function getLanguagesFilterLabel() {
    ...
    if (filters.languages.length === 1)
      return LANGUAGES.find((l) => l.code === filters.languages![0])?.name || 'Languages'
  ...
  function renderFilterDropdown(
    key: string,
    items: { value: string; label?: string; name?: string; code?: string }[],
    field: 'specialty_tags' | 'content_style_tags' | 'preferred_platforms' | 'languages'
  ) {
      ...
      <div className="cl-filter-dropdown" style={getFilterDropdownPosition(key)} onClick={(e) => e.stopPropagation()}>
        {items.map((item) => {
          ...
          return (
            <button
              key={val}
              className={`cl-filter-dropdown__item ${isSelected ? 'cl-filter-dropdown__item--active' : ''}`}
              onClick={() => toggleArrayFilter(field, val)}
            >
              <span className={`cl-filter-dropdown__check ${isSelected ? 'cl-filter-dropdown__check--active' : ''}`}>
  ...
  function renderExperienceDropdown() {
    ...
    return createPortal(
      <div
        ...
        style={getFilterDropdownPosition('experience')}
        onClick={(e) => e.stopPropagation()}
      >
        <button
          className={`cl-filter-dropdown__item ${!filters.experience_level || filters.experience_level === 'any' ? 'cl-filter-dropdown__item--active' : ''}`}
          onClick={() => {
            updateFilters({ experience_level: 'any' })
            setOpenFilter(null)
          }}
        >
          Any Experience
        ...
        {EXPERIENCE_LEVELS.map((level) => (
          <button
            key={level.value}
            className={`cl-filter-dropdown__item ${filters.experience_level === level.value ? 'cl-filter-dropdown__item--active' : ''}`}
            onClick={() => {
              updateFilters({ experience_level: level.value })
              setOpenFilter(null)
            }}
          >
            {level.label}
  ...
  if (loading && activeView === 'directory') {
    return (
      <PageLayout icon={Scissors} title="Clippers">
      ...
      actions={
        <div className="cl-view-toggle">
          <button
            ...
            className={`cl-view-toggle__btn ${activeView === 'directory' ? 'cl-view-toggle__btn--active' : ''}`}
          >
          ...
          <button
            ...
            className={`cl-view-toggle__btn ${activeView === 'leaderboard' ? 'cl-view-toggle__btn--active' : ''}`}
          >
    ...
    >
      <div className="cl">
        <div className="cl-heading">
          ...
          <p className="cl-heading__subtitle">
          ...
          <>
            <div className="cl-toolbar">
              <div className="cl-period-toggle">
                <button
                  onClick={() => switchLeaderboardPeriod('weekly')}
                  className={`cl-period-toggle__btn ${leaderboardPeriod === 'weekly' ? 'cl-period-toggle__btn--active' : ''}`}
                >
                  Weekly
                </button>
                <button
                  onClick={() => switchLeaderboardPeriod('monthly')}
                  className={`cl-period-toggle__btn ${leaderboardPeriod === 'monthly' ? 'cl-period-toggle__btn--active' : ''}`}
                >
                  Monthly
                </button>
              </div>
            ...
            {loadingLeaderboard ? (
              ...
              <div className="cl-leaderboard-list">
                {leaderboardEntries.map((entry, index) => (
                  <div
                    key={entry.id}
                    className={`cl-leaderboard-card ${index === 0 ? 'cl-leaderboard-card--gold' : index === 1 ? 'cl-leaderboard-card--silver' : index === 2 ? 'cl-leaderboard-card--bronze' : ''}`}
                  >
                    <div
                      className={`cl-leaderboard-card__indicator ${index === 0 ? 'cl-leaderboard-card__indicator--gold' : index === 1 ? 'cl-leaderboard-card__indicator--silver' : index === 2 ? 'cl-leaderboard-card__indicator--bronze' : ''}`}
                    />
                    <div className="cl-leaderboard-card__content">
                      <div
                        className={`cl-leaderboard-card__rank ${index === 0 ? 'cl-leaderboard-card__rank--gold' : index === 1 ? 'cl-leaderboard-card__rank--silver' : index === 2 ? 'cl-leaderboard-card__rank--bronze' : ''}`}
                      >
                        {index + 1}
                      </div>
                      <div className="cl-leaderboard-card__avatar">
                        {entry.clipper_profile?.avatar_url ? (
                          <img src={entry.clipper_profile.avatar_url} className="cl-leaderboard-card__avatar-img" />
                        ) : (
                          <User className="w-6 h-6 text-zinc-500" />
                        )}
                      </div>
                      <div className="cl-leaderboard-card__info">
                        <div className="cl-leaderboard-card__name">
                          {entry.clipper_profile?.display_name || 'Anonymous Clipper'}
                          {entry.clipper_profile?.is_verified && (
          ...
          <>
            ...
            <div className="cl-toolbar">
              <label className="cl-filter-toggle">
                <input
                  type="checkbox"
                  className="cl-filter-toggle__input"
                  checked={!!filters.looking_for_work}
                  onChange={(e) => updateFilters({ looking_for_work: e.target.checked || undefined })}
                />
                <span className="cl-filter-toggle__switch">
                  <span className="cl-filter-toggle__handle" />
                </span>
              ...
              <label className="cl-filter-toggle">
                <input
                  ...
                  onChange={(e) => updateFilters({ verified_only: e.target.checked || undefined })}
                />
                <span className="cl-filter-toggle__switch">
                  <span className="cl-filter-toggle__handle" />
                </span>
              ...
              <button
                ref={(el) => {
                  if (el) filterBtnRefs.current.set('experience', el)
                  else filterBtnRefs.current.delete('experience')
                }}
                className={`cl-filter-btn ${openFilter === 'experience' ? 'cl-filter-btn--open' : ''}`}
                onClick={(e) => {
                  e.stopPropagation()
              ...
              >
                ...
                <ChevronDown
                  className={`cl-filter-btn__chevron ${openFilter === 'experience' ? 'cl-filter-btn__chevron--open' : ''}`}
                />
              </button>
              ...
              <button
                ref={(el) => {
                  if (el) filterBtnRefs.current.set('specialties', el)
                  else filterBtnRefs.current.delete('specialties')
                }}
                className={`cl-filter-btn cl-filter-btn--wide ${openFilter === 'specialties' ? 'cl-filter-btn--open' : ''}`}
                onClick={(e) => {
                  e.stopPropagation()
                  setOpenFilter(openFilter === 'specialties' ? null : 'specialties')
                }}
              >
                <span>{getSpecialtiesFilterLabel()}</span>
                <ChevronDown
                  className={`cl-filter-btn__chevron ${openFilter === 'specialties' ? 'cl-filter-btn__chevron--open' : ''}`}
                />
              </button>
              ...
              <button
                ref={(el) => {
                  if (el) filterBtnRefs.current.set('content_style', el)
                  else filterBtnRefs.current.delete('content_style')
                }}
                className={`cl-filter-btn cl-filter-btn--wide ${openFilter === 'content_style' ? 'cl-filter-btn--open' : ''}`}
                onClick={(e) => {
                  e.stopPropagation()
                  setOpenFilter(openFilter === 'content_style' ? null : 'content_style')
                }}
              >
                <span>{getContentStyleFilterLabel()}</span>
                <ChevronDown
                  className={`cl-filter-btn__chevron ${openFilter === 'content_style' ? 'cl-filter-btn__chevron--open' : ''}`}
                />
              </button>
              ...
              <button
                ref={(el) => {
                  if (el) filterBtnRefs.current.set('platforms', el)
                  else filterBtnRefs.current.delete('platforms')
                }}
                className={`cl-filter-btn ${openFilter === 'platforms' ? 'cl-filter-btn--open' : ''}`}
                onClick={(e) => {
                  e.stopPropagation()
                  setOpenFilter(openFilter === 'platforms' ? null : 'platforms')
                }}
              >
                <span>{getPlatformsFilterLabel()}</span>
                <ChevronDown
                  className={`cl-filter-btn__chevron ${openFilter === 'platforms' ? 'cl-filter-btn__chevron--open' : ''}`}
                />
              </button>
              ...
              <button
                ref={(el) => {
                  if (el) filterBtnRefs.current.set('languages', el)
                  else filterBtnRefs.current.delete('languages')
                }}
                className={`cl-filter-btn ${openFilter === 'languages' ? 'cl-filter-btn--open' : ''}`}
                onClick={(e) => {
                  e.stopPropagation()
              ...
              >
                ...
                <ChevronDown
                  className={`cl-filter-btn__chevron ${openFilter === 'languages' ? 'cl-filter-btn__chevron--open' : ''}`}
                />
              </button>
              {renderFilterDropdown('languages', LANGUAGES as any, 'languages')}

              <div className="cl-toolbar__spacer" />
              <button className="cl-clear-btn" onClick={clearFilters} disabled={!hasActiveFilters}>
                <X className="w-3 h-3" />
                Clear
              </button>
            </div>
                ...
                {clippers.map((clipper) => (
                  <div key={clipper.id} className="cl-card">
                    ...
                    <div className="cl-card__status-bar">
                      <div
                        className={`cl-card__availability ${clipper.looking_for_work ? 'cl-card__availability--available' : ''}`}
                      >
                        {clipper.looking_for_work ? (
                      ...
                      {clipper.response_time_hours != null && (
                        <div className="cl-card__response-time">
                    ...
                    <div className="cl-card__main">
                      ...
                      <div className="cl-card__header">
                        <div className="cl-card__avatar">
                          {clipper.avatar_url ? (
                            <img src={clipper.avatar_url} className="cl-card__avatar-img" />
                          ) : (
                            <div className="cl-card__avatar-fallback">
                              <User className="w-[26px] h-[26px] text-zinc-500 opacity-60" />
                            </div>
                          )}
                        </div>
                        <div className="cl-card__info">
                          ...
                          {clipper.experience_level && (
                            <div className="cl-card__level">{getExperienceLevelLabel(clipper.experience_level)}</div>
                          )}
                          <p className={`cl-card__bio ${!clipper.bio ? 'cl-card__bio--empty' : ''}`}>
                        ...
                        {clipper.preferred_platforms?.length > 0 && (
                          <div className="cl-card__platforms">
                            {clipper.preferred_platforms.slice(0, 5).map((p) => (
                              <span key={p} className={`cl-platform-icon ${getPlatformIconClass(p)}`} title={p}>
                                {getPlatformLetter(p)}
                              </span>
                            ))}
                          </div>
                      ...
                      <div className="cl-card__stats-row">
                        <div className="cl-card__stat">
                          <Video className="w-[13px] h-[13px] text-zinc-500 opacity-70" />
                          <span className="cl-card__stat-value">{clipper.total_clips_delivered || 0}</span>
                          <span className="cl-card__stat-label">clips</span>
                        </div>
                        <div className="cl-card__stat">
                          <Star className="w-[13px] h-[13px] text-amber-500" />
                          <span className="cl-card__stat-value">{clipper.total_endorsements || 0}</span>
                          <span className="cl-card__stat-label">reviews</span>
                        </div>
                        <div className="cl-card__stat">
                          <Trophy className="w-[13px] h-[13px] text-cyan-400" />
                          <span className="cl-card__stat-value">{clipper.total_campaigns_completed || 0}</span>
                          <span className="cl-card__stat-label">campaigns</span>
                        </div>
                      </div>
                        ...
                        <div className="cl-card__portfolio">
                          {clipper.portfolio_clips.slice(0, 2).map((clip) => (
                            <div key={clip.id} className="cl-card__portfolio-thumb">
                              {clip.thumbnail_url ? (
                                <img src={clip.thumbnail_url} className="cl-card__portfolio-img" />
                              ) : (
                                <div className="cl-card__portfolio-placeholder">
                                  <Play className="w-4 h-4 text-zinc-500 opacity-50" />
                                </div>
                              )}
                            </div>
                          ))}
                          {clipper.portfolio_clips.length > 2 && (
                            <span className="cl-card__portfolio-more">+{clipper.portfolio_clips.length - 2} more</span>
                          )}
                        </div>
                      )}
                      ...
                      <div className="cl-card__tags-row">
                        <div className="cl-card__tags">
                          {clipper.specialty_tags?.slice(0, 2).map((tag) => (
                            <span key={tag} className="cl-card__tag">
                              {getSpecialtyTagLabel(tag)}
                            </span>
                          ))}
                          {clipper.specialty_tags?.length > 2 && (
                            <span className="cl-card__tag-more">+{clipper.specialty_tags.length - 2}</span>
                          )}
                        </div>
                        {clipper.languages?.length > 0 && (
                          <div className="cl-card__languages">
                            {clipper.languages.slice(0, 3).map((lang) => (
                              <span key={lang} className="cl-card__lang-badge" title={getLanguageName(lang)}>
                                {getLanguageFlag(lang)}
                              </span>
                            ))}
                            {clipper.languages.length > 3 && (
                              <span className="cl-card__lang-more">+{clipper.languages.length - 3}</span>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="cl-card__actions">
                      <a
                        href={`/dashboard/org/${organizationId}/messages?to=${clipper.user_id}`}
                        className="cl-card__action-btn cl-card__action-btn--message"
                      ...
                      <button
                        className="cl-card__action-btn cl-card__action-btn--invite"
                        onClick={() => openInviteDialog(clipper)}
                      >
          ...
          >
            <div className="om-dialog om-dialog--cyan" style={{ maxWidth: 420 }}>
              ...
              <div className="om-dialog__header">
                <button className="om-dialog__close" onClick={() => setShowInviteDialog(false)}>
                  <X size={18} />
                </button>
                ...
                <p className="om-dialog__subtitle">
                ...
                {loadingCampaigns ? (
                  ...
                  <div className="cl-invite__campaign-list">
                    {organizationCampaigns.map((campaign) => (
                      <button
                        key={campaign.id}
                        className={`cl-invite__campaign-option ${selectedCampaignId === campaign.id ? 'cl-invite__campaign-option--selected' : ''}`}
                        onClick={() => setSelectedCampaignId(campaign.id)}
                      >
                        <div className="cl-invite__campaign-info">
                          <span className="cl-invite__campaign-title">{campaign.title}</span>
                          <span className="cl-invite__campaign-meta">
              ...
              <div className="om-dialog__footer">
                <button className="om-dialog__btn om-dialog__btn--secondary" onClick={() => setShowInviteDialog(false)}>
                  Cancel
                </button>
                <button
                  className="om-dialog__btn om-dialog__btn--primary"
                  disabled={!selectedCampaignId || organizationCampaigns.length === 0}
