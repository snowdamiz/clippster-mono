...
export function OrgMembers() {
  ...
  const [editMemberData, setEditMemberData] = useState<{
    ...
    email: string
    password: string
  }>({ member: null, name: '', email: '', password: '' })
  ...
  async function toggleMemberRestriction(member: OrganizationMember) {
    ...
    try {
      const response = await api.put<any>(`/organizations/${organizationId}/members/${member.user_id}/restrictions`, {
        ...
        isAdmin && (
          <div className="flex items-center gap-3">
            {hasActiveSubscription && seatsRemaining != null && (
              <span
                className={`inline-flex items-center px-3 py-1.5 text-[0.6875rem] font-semibold rounded-md uppercase tracking-wider ${
                  seatsRemaining === 0
    ...
    >
      <div className="om">
        ...
        <div className="om-heading">
          ...
          <p className="om-heading__subtitle">
        ...
        <div className="om-stats">
          ...
          <div className="om-stats__card">
            ...
            <div className="om-stats__inner">
              <div className="om-stats__icon om-stats__icon--amber">
          ...
          <div className="om-stats__card">
            ...
            <div className="om-stats__inner">
              <div className="om-stats__icon om-stats__icon--red">
        ...
        {isAdmin && invitations.length > 0 && (
          <section className="om-section">
            <div className="om-section__header">
              <div className="om-section__header-icon om-section__header-icon--amber">
              ...
              {invitations.map((inv) => (
                <div key={inv.id} className="om-invitation">
                  <div className="om-invitation__inner">
                    <div className="om-invitation__icon">
                      <Mail className="w-[18px] h-[18px] text-amber-500" />
                    </div>
                    <div className="om-invitation__info">
                      <div className="om-invitation__email">{inv.email}</div>
                      <div className="om-invitation__meta">
                        <span
                          className={`om-invitation__role ${inv.role === 'admin' ? 'om-invitation__role--admin' : ''}`}
                        >
                          {inv.role}
                        </span>
                        <span className="om-invitation__dot" />
                        <span className="om-invitation__expires">
                    ...
                    <div className="om-invitation__actions">
                      <button
                        ...
                        className="om-invitation__action"
                      >
                        <RefreshCw
                          className={`w-3.5 h-3.5 ${resendingInvitationId === inv.id ? 'animate-spin' : ''}`}
                        />
                      </button>
                      <button
                        onClick={() => cancelInvitation(inv.id)}
                        title="Cancel invitation"
                        className="om-invitation__action om-invitation__action--danger"
                      >
        ...
        <section className="om-section">
          <div className="om-section__header">
            <div className="om-section__header-icon om-section__header-icon--blue">
          ...
          {members.length > 0 ? (
            <div className="om-members-list">
              {members.map((member) => (
                <div
                  key={member.id}
                  className={`om-member ${member.role === 'owner' ? 'om-member--owner' : member.role === 'admin' ? 'om-member--admin' : ''}`}
                >
                    ...
                    <div className="om-member__row">
                      ...
                      <div className="om-member__avatar">
                        {member.user?.avatar_url && !failedAvatars.has(member.user_id) ? (
                          <img
                            ...
                            className="om-member__avatar-img"
                            referrerPolicy="no-referrer"
                            onError={() => handleAvatarError(member.user_id)}
                          />
                        ) : (
                          <div className="om-member__avatar-fallback">
                            <User className="w-5 h-5 text-zinc-500 opacity-60" />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="om-member__credits">
                        <div className="om-member__credit">
                          ...
                          <span className="om-member__credit-value">
                        ...
                        <div className="om-member__credit om-member__credit--muted">
                          ...
                          <span className="om-member__credit-value om-member__credit-value--muted">
                      ...
                      {/* Role Badge */}
                      <span
                        className={`om-member__role ${member.role === 'owner' ? 'om-member__role--owner' : member.role === 'admin' ? 'om-member__role--admin' : ''}`}
                      >
                      ...
                      {isAdmin && member.role !== 'owner' && (
                        <div className="om-member__actions">
                          <button
                            ref={(el) => {
                              if (el) memberMenuButtonRefs.current.set(member.user_id, el)
                              else memberMenuButtonRefs.current.delete(member.user_id)
                            }}
                            className={`om-member__menu-btn ${openMemberMenuId === member.user_id ? 'om-member__menu-btn--active' : ''}`}
                            title="Member actions"
                            onClick={(e) => {
                              e.stopPropagation()
                              setOpenMemberMenuId(openMemberMenuId === member.user_id ? null : member.user_id)
                            }}
                          >
                          ...
                          {openMemberMenuId === member.user_id &&
                            createPortal(
                              <div
                                className="om-dropdown"
                                style={getMemberMenuPosition(member.user_id)}
                                onClick={(e) => e.stopPropagation()}
                              >
                                {isOrgCreatedUser(member) && (
                                  <button
                                    className="om-dropdown__item om-dropdown__item--edit"
                                    onClick={() => {
                                      openEditMemberDialogFor(member)
                                      setOpenMemberMenuId(null)
                                    }}
                                  >
                                ...
                                <button
                                  className="om-dropdown__item"
                                  onClick={() => {
                                    openRoleDialogFor(member)
                                    setOpenMemberMenuId(null)
                                  }}
                                >
                                ...
                                <div className="om-dropdown__toggle-item">
                                  <div className="om-dropdown__toggle-label">
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation()
                                      toggleMemberRestriction(member)
                                    }}
                                  >
                                ...
                                <div className="om-dropdown__divider" />
                                <button
                                  className="om-dropdown__item om-dropdown__item--danger"
                                  onClick={() => {
                                    confirmRemoveMemberFor(member)
                                    setOpenMemberMenuId(null)
                                  }}
                                >
            ...
            <div className="om-empty">
              <div className="om-empty__icon-wrapper">
              ...
              {isAdmin && (
                <button onClick={() => setShowInviteDialog(true)} className="om-empty__btn">
      ...
      <InviteMemberDialog
        ...
        onSuccess={() => loadOrganization(true)}
      />
      ...
      {showSeatLimitDialog &&
        createPortal(
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) setShowSeatLimitDialog(false)
            }}
          >
            <div className="om-dialog om-dialog--warning">
              ...
              <div className="om-dialog__header">
                <button className="om-dialog__close" onClick={() => setShowSeatLimitDialog(false)}>
                  <X size={18} />
                </button>
                <div className="om-dialog__icon om-dialog__icon--warning">
                  <Users size={24} />
                </div>
              ...
              <div className="om-dialog__footer">
                <button
                  ...
                  onClick={() => setShowSeatLimitDialog(false)}
                >
          ...
          >
            <div className="om-dialog om-dialog--cyan">
              ...
              <div className="om-dialog__content">
                <div className="om-dialog__role-change">
                  <div className="om-dialog__role-item">
                    ...
                    <span
                      className={`om-dialog__role-value ${roleDialogMember?.role === 'admin' ? 'om-dialog__role-value--admin' : ''}`}
                    >
                      {roleDialogMember?.role}
                    </span>
                  </div>
                  <div className="om-dialog__role-arrow">
                    <ArrowDown size={18} />
                  </div>
                  <div className="om-dialog__role-item">
                    <span className="om-dialog__role-label">New Role</span>
                    <span
                      className={`om-dialog__role-value ${roleDialogNewRole === 'admin' ? 'om-dialog__role-value--admin' : ''}`}
                    >
                      {roleDialogNewRole}
                    </span>
                  </div>
              ...
              <div className="om-dialog__footer">
                ...
                <button className="om-dialog__btn om-dialog__btn--primary" onClick={confirmRoleChange}>
                  Confirm Change
                </button>
              </div>
            </div>
      ...
      {showEditMemberDialog &&
        createPortal(
          <div
            className="om-dialog__overlay"
            onClick={(e) => {
              if (e.target === e.currentTarget && !editMemberSaving) {
                setShowEditMemberDialog(false)
                setEditMemberData({ member: null, name: '', email: '', password: '' })
              }
            }}
                ...
                <div className="om-dialog__content">
                  <div className="om-dialog__field">
                    ...
                    <input
                      ...
                      onChange={(e) => setEditMemberData((d) => ({ ...d, name: e.target.value }))}
                      type="text"
                      placeholder="Enter name"
                      className="om-dialog__input"
                    />
                  </div>
                  <div className="om-dialog__field">
                    ...
                    <input
                      ...
                      onChange={(e) => setEditMemberData((d) => ({ ...d, email: e.target.value }))}
                      type="email"
                  ...
                  <div className="om-dialog__field">
                    <label className="om-dialog__label">
                      New Password <span className="om-dialog__label-hint">(leave empty to keep current)</span>
                    </label>
                    <div className="om-dialog__input-group">
                      <input
                        value={editMemberData.password}
                        onChange={(e) => setEditMemberData((d) => ({ ...d, password: e.target.value }))}
                      ...
                      <button
                        ...
                        className="om-dialog__input-toggle"
                      >
      ...
      {showRemoveMemberDialog &&
        createPortal(
          <div
            className="om-dialog__overlay"
            onClick={(e) => {
              if (e.target === e.currentTarget && !removeMemberProcessing) {
                setShowRemoveMemberDialog(false)
                setRemoveMemberDialogMember(null)
              }
            }}
              ...
              <div className="om-dialog__content">
                <div className="om-dialog__member-card">
                  <div className="om-dialog__member-avatar">
                    {removeMemberDialogMember?.user?.avatar_url ? (
                      <img
                        ...
                        className="om-dialog__member-avatar-img"
                        referrerPolicy="no-referrer"
                      />
                    ) : (
                  ...
                  <div className="om-dialog__member-info">
                    <div className="om-dialog__member-name">
                  ...
                  <span
                    className={`om-dialog__role-value ${removeMemberDialogMember?.role === 'admin' ? 'om-dialog__role-value--admin' : ''}`}
                  >
