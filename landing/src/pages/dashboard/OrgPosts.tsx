...
import { listExternalPosts, syncOrgAnalytics, formatRelativeTime } from '@/services/schedulingApi'
...
import { FileText, Eye, Heart, MessageSquare, RefreshCw, ExternalLink, Instagram, Globe, TrendingUp, Clock, User, ChevronDown, Video, Filter, BarChart3 } from 'lucide-react'
...
function FilterDropdown({ label, value, options, onChange }: { label: string; value: string; options: { value: string; label: string; dot?: string }[]; onChange: (v: string) => void }) {
...
const statColorMap: Record<string, { indicator: string; iconBg: string; iconColor: string; valueColor: string }> = {
  cyan: { indicator: 'from-cyan-500 to-cyan-600', iconBg: 'bg-cyan-500/15', iconColor: 'text-cyan-500', valueColor: 'text-cyan-500' },
  violet: { indicator: 'from-violet-500 to-violet-600', iconBg: 'bg-violet-500/15', iconColor: 'text-violet-500', valueColor: 'text-violet-500' },
  pink: { indicator: 'from-pink-500 to-pink-600', iconBg: 'bg-pink-500/15', iconColor: 'text-pink-500', valueColor: 'text-pink-500' },
  emerald: { indicator: 'from-emerald-500 to-emerald-600', iconBg: 'bg-emerald-500/15', iconColor: 'text-emerald-500', valueColor: 'text-emerald-500' },
}
...
export function OrgPosts() {
  const { loadOrganization, organizationId, isAdmin, members, creatorProfiles, loadCreatorProfiles, profilesLoaded } = useOrganization()
  ...
  const [summary, setSummary] = useState<AnalyticsSummary>({ total_posts: 0, total_views: 0, total_likes: 0, total_reach: 0 })
  const toast = useToast()
  const perPage = 20

  useEffect(() => { loadOrganization(); if (!profilesLoaded) loadCreatorProfiles() }, [loadOrganization])

  // Sync analytics on mount
  useEffect(() => {
    if (!organizationId) return
    syncOrgAnalytics(organizationId).then(() => {
      setTimeout(() => { loadPosts(); loadSummary() }, 2000)
    }).catch(() => {})
  }, [organizationId])
  ...
  const loadPosts = async (p = 1) => {
    ...
    try {
      ...
      const [scheduledRes, externalRes, campaignRes] = await Promise.all([
        ...
        listExternalPosts(organizationId, { status: statusFilter || undefined, limit: perPage, offset: (p - 1) * perPage }),
        listOrganizationCampaignSubmissions(organizationId, { status: statusFilter || undefined, platform: platformFilter || undefined, limit: perPage, offset: (p - 1) * perPage }),
      ])
      ...
      if (scheduledRes.success) {
        for (const post of scheduledRes.posts) {
          unified.push({
            id: post.id, type: 'scheduled', platform: post.platform, status: post.status,
            post_url: post.post_url, caption: post.caption,
            view_count: post.analytics?.view_count || 0, like_count: post.analytics?.like_count || 0, comment_count: post.analytics?.comment_count || 0,
            creator_profile: post.creator_profile ? { id: post.creator_profile.id, name: post.creator_profile.name } : null,
            campaign: null, social_account: post.social_account,
            submitted_by: post.submitted_by ? { id: post.submitted_by.id, email: post.submitted_by.email, name: post.submitted_by.name } : null,
            author_username: null, author_name: null, author_profile_image: null,
      ...
      if (externalRes.success) {
        for (const post of externalRes.submissions) {
          unified.push({
            id: post.id, type: 'external', platform: post.platform, status: 'published',
            post_url: post.post_url, caption: post.caption,
            view_count: post.analytics?.view_count || 0, like_count: post.analytics?.like_count || 0, comment_count: post.analytics?.comment_count || 0,
            creator_profile: post.creator_profile ? { id: post.creator_profile.id, name: post.creator_profile.name } : null,
            campaign: post.campaign, social_account: null,
            submitted_by: post.submitted_by ? { id: post.submitted_by.id, email: post.submitted_by.email, name: post.submitted_by.name } : null,
            author_username: post.author_username, author_name: post.author_name, author_profile_image: post.author_profile_image,
            inserted_at: post.inserted_at,
          })
        }
      }
      ...
      if (campaignRes.success) {
        for (const sub of campaignRes.submissions) {
          unified.push({
            id: sub.id, type: 'campaign', platform: sub.platform,
            status: (sub.status === 'verified' || sub.status === 'paid') ? 'published' : sub.status,
            post_url: sub.clip_url || sub.post_url || null, caption: sub.caption || null,
            view_count: sub.view_count || 0, like_count: sub.like_count || 0, comment_count: sub.comment_count || 0,
            creator_profile: sub.creator_profile ? { id: sub.creator_profile.id, name: sub.creator_profile.name } : null,
            campaign: sub.campaign ? { id: sub.campaign.id, name: sub.campaign.title || sub.campaign.name } : null,
            social_account: null,
            submitted_by: sub.user ? { id: sub.user.id, email: sub.user.email, name: sub.user.display_name || sub.user.name } : null,
            author_username: sub.author_username || null, author_name: sub.author_name || null, author_profile_image: sub.author_profile_image || null,
  ...
  const loadSummary = async () => {
    ...
    try {
      ...
      let tp = 0, tv = 0, tl = 0, tr = 0
      if (schedRes.success && schedRes.summary) {
        tp = schedRes.summary.total_posts || 0; tv = schedRes.summary.total_views || 0
        tl = schedRes.summary.total_likes || 0; tr = schedRes.summary.total_reach || 0
      }
      if (extRes.success && extRes.submissions) {
        tp += extRes.submissions.length
        for (const p of extRes.submissions) { tv += p.analytics?.view_count || 0; tl += p.analytics?.like_count || 0 }
      }
      if (campRes.success && campRes.submissions) {
        tp += campRes.submissions.length
        for (const s of campRes.submissions) { tv += s.view_count || 0 }
      }
      setSummary({ total_posts: tp, total_views: tv, total_likes: tl, total_reach: tr })
    } catch {}
  }

  useEffect(() => { loadPosts(); loadSummary() }, [organizationId, statusFilter, platformFilter])
    ...
    <PageLayout icon={FileText} title="Post Analytics">
      <div className="p-6 flex flex-col gap-8 max-w-[1400px] mx-auto w-full">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map(i => <Skeleton key={i} className="h-20 rounded-[10px]" />)}
        </div>
  ...
  return (
    <PageLayout icon={FileText} title="Post Analytics" actions={
      <button onClick={handleRefresh} disabled={syncing} className="flex items-center gap-2 h-8 px-3.5 text-xs font-semibold rounded-md bg-cyan-400 text-[#0a0a0b] border-none cursor-pointer transition-opacity duration-150 hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div className="p-6 flex flex-col gap-8 max-w-[1400px] mx-auto w-full">
        ...
        <div className="mb-2">
          ...
          <p className="text-sm text-zinc-500 leading-relaxed">Track performance metrics and engagement for your social media posts</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {statCards.map(card => {
            ...
            return (
              <div key={card.key} className="flex bg-zinc-900/50 border border-zinc-800 rounded-[10px] overflow-hidden hover:border-zinc-700 transition-all hover:shadow-[0_4px_20px_rgba(0,0,0,0.15)]">
                <div className={`w-1 shrink-0 bg-gradient-to-b ${colors.indicator}`} />
                <div className="flex items-center gap-3.5 flex-1 py-4 px-5">
                  <div className={`w-10 h-10 rounded-[10px] flex items-center justify-center shrink-0 ${colors.iconBg}`}>
                  ...
                  <span className={`text-[1.75rem] font-bold tabular-nums leading-none tracking-[-0.02em] ${colors.valueColor}`}>{formatNumber(val)}</span>
                </div>
              </div>
            )
          })}
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 py-4 px-5 bg-zinc-900/50 border border-zinc-800 rounded-[10px]">
            <FilterDropdown label="All Status" value={statusFilter} options={statusOptions} onChange={v => setStatusFilter(v)} />
            <FilterDropdown label="All Platforms" value={platformFilter} options={platformOptions} onChange={v => setPlatformFilter(v)} />
        ...
        <section className="flex flex-col gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <span className="inline-flex items-center justify-center min-w-[24px] h-6 px-2 text-xs font-semibold text-violet-500 bg-violet-500/15 rounded-full">{total}</span>
          </div>
        {posts.length === 0 ? (
          <EmptyState icon={FileText} title="No Posts" description={statusFilter || platformFilter ? 'No posts match these filters' : 'Posts published to social media will appear here with analytics'} />
        ) : (
          <div className="space-y-2">
            {posts.map(post => {
              const PlatformIcon = getPlatformIcon(post.platform)
              return (
                <div key={`${post.type}-${post.id}`} className="flex bg-zinc-900/50 border border-zinc-800 rounded-[10px] overflow-hidden hover:border-white/[0.12] transition-all hover:shadow-[0_4px_16px_rgba(0,0,0,0.15)]">
                  ...
                  <div className="flex items-center gap-4 flex-1 py-3.5 px-4">
                    {/* Platform badge */}
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${getPlatformBgClass(post.platform)}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {post.campaign && (
                          <span className="text-[11px] font-medium text-violet-400 bg-violet-500/15 px-1.5 py-0.5 rounded">{post.campaign.name}</span>
                        )}
                        {post.type === 'campaign' && (
                          <span className="text-[10px] font-semibold uppercase text-cyan-400 bg-cyan-500/15 px-1.5 py-0.5 rounded tracking-wide">Campaign Submission</span>
                        )}
                      </div>
                      <div className="flex items-center gap-3 text-[11px] text-zinc-500">
                        {post.submitted_by && (
                          <span className="flex items-center gap-1"><User className="w-3 h-3" /> {post.submitted_by.name || post.submitted_by.email}</span>
                        )}
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {formatDate(post.inserted_at)}</span>
                      </div>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-6 pl-4 border-l border-zinc-800 shrink-0">
                      <div className="flex flex-col items-center min-w-[56px]">
                        ...
                        <span className="text-base font-bold text-white tabular-nums">{formatNumber(post.view_count)}</span>
                        <span className="text-[10px] text-zinc-500 uppercase tracking-wide">Views</span>
                      </div>
                      <div className="flex flex-col items-center min-w-[56px]">
                        <Heart className="w-4 h-4 text-zinc-500 mb-0.5" />
                        <span className="text-base font-bold text-white tabular-nums">{formatNumber(post.like_count)}</span>
                      ...
                      <div className="flex flex-col items-center min-w-[56px]">
                        ...
                        <span className="text-base font-bold text-white tabular-nums">{formatNumber(post.comment_count)}</span>
