...
export default function BrandingProfilesScreen() {
  ...
  return (
    <View className="flex-1 bg-background">
      <ScreenHeader title="Creator branding" showBack />
      <ScrollView contentContainerClassName="gap-4 px-4 py-4 pb-10">
        ...
        <View className="flex-row flex-wrap gap-2">
          <Button title={syncing ? 'Syncing…' : 'Sync now'} onPress={() => void handleSync()} disabled={syncing} />
          <Button title="New profile" variant="outline" onPress={() => void handleCreateProfile()} disabled={uploading} />
        </View>

        <View className="gap-2">
          <Text className="text-xs font-semibold uppercase tracking-wide text-muted">Upload assets</Text>
          <View className="flex-row flex-wrap gap-2">
            <Button title="Watermark" variant="outline" onPress={() => void handleUploadAsset('watermark')} disabled={uploading} />
            <Button title="Intro" variant="outline" onPress={() => void handleUploadAsset('intro')} disabled={uploading} />
            <Button title="Outro" variant="outline" onPress={() => void handleUploadAsset('outro')} disabled={uploading} />
          </View>
        </View>
        ...
        {loading ? (
          ...
          <Text className="text-sm text-muted">No personal branding profiles yet. Create one or sync from desktop.</Text>
        ) : (
